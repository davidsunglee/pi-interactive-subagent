...
3. **Resume awareness (P6)** — the orchestration layer tracks which live orchestrations own which session ids, so a resume (standalone) that targets an orch-owned session feeds its result back into the original orchestration's blocked slot.
...
- **No session-id substitution in task prompts.** `{previous}` remains text-only, bound to the prior task's `finalMessage`.
...
- **Blocked → running (via resume).** The resumed child uses the same session id; accumulators extend cumulatively.
...
```
sessionId → (orchestrationId, taskIndex)
```

Populated when each task launches — session id is available once the child emits its first message in both backends. Cleared when the task reaches a terminal state. Load-bearing for both blocked-state routing and resume re-ingestion.

### Detecting a block

The existing Claude Stop-hook plugin already emits structured sentinels distinguishing `subagent_done` from `caller_ping`. Extend the completion watcher in `pi-extension/subagents/` to surface the `caller_ping` case as a distinct event rather than folding it into normal completion. On receiving the event for a child whose `sessionId` is in the ownership map, the registry transitions the task:
...
The parent calls the standalone `subagent_resume` tool directly with the blocked session's `sessionId` and the answer as the follow-up prompt. This is the only unblock path; no orchestration-level unblock tool is introduced.
...
2. On terminal completion (success, failure, or cancellation), the registry matches `sessionId → (orchestrationId, taskIndex)` and updates the original orchestration's blocked slot with the resumed child's result.
3. State transitions `blocked → completed | failed | cancelled`. Usage / transcript extend cumulatively (same session id continues the accumulators).
...
| `subagent_resume` | pre-existing | Resume a session (unchanged behavior; new side effect: orch-owned sessions route results back to the owning orch) |
...
| `blocked` | Phase 2 | blocked task details (orchestrationId, taskIndex, sessionId, message) |
...
- **Unit:** ownership-map populate/clear on task lifecycle; sessionId lookup routing to the right `(orchestrationId, taskIndex)`.
- **Integration — single block/resume cycle:** serial run with `wait: false`, task 2 calls `caller_ping`, blocked steer-back fires, parent issues standalone `subagent_resume`, task 2 transitions `blocked → completed`, sequence continues to task 3, aggregated completion fires.
...
- **Bare-subagent cancellation** is a larger conversation; if pursued later, the natural consolidation is to make `subagent_run_cancel` polymorphic on `{ orchestrationId? sessionId? }`.
