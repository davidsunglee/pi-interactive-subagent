# Claude Pane Interactive Subagents — Design
...
5. `pi-extension/subagents/index.ts` *(modified)* — Claude pane launch path:
   ...
   - `buildClaudeCmdParts` always injects `mcp__pi-subagent__subagent_done` into the `--tools` allowlist whenever the flag is emitted (symmetric to how `resolvePiToolsArg` always reserves `caller_ping,subagent_done` on the pi path).
...
`buildClaudeCmdParts` already emits `--tools <list>` only when `effectiveTools` is set (mapping pi tool names → Claude tool names via `PI_TO_CLAUDE_TOOLS`). Change: when emitting that list, unconditionally include `mcp__pi-subagent__subagent_done` alongside the mapped builtins. When `effectiveTools` is unset, no `--tools` flag is emitted and Claude's default allowlist permits MCP tools.
...
1. `--tools` always contains `mcp__pi-subagent__subagent_done` whenever it is emitted, alongside mapped builtins.
2. `--tools` is omitted entirely when no `effectiveTools` is set.
3. System-prompt addendum present and matches the `auto-exit: false` form when `agentDefs.autoExit === false`.
4. System-prompt addendum present and matches the `auto-exit: true` form when `agentDefs.autoExit === true`.
5. Addendum appended after `spec.identity` (not replacing) — verify ordering.
6. When `spec.identity` is null/empty, the addendum still flows: `--append-system-prompt` is emitted with the addendum as its sole content.
7. When `effectiveCli !== "claude"`, `claudeCompletionAddendum` is `null` on the resolved spec.
