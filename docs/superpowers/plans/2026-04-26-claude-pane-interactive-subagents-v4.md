...
```ts
      ...
      // pollForExit's contract is unchanged — this wait runs after it.
      if (running.sentinelFile) {
...
In `launchSubagent`'s Claude branch, before constructing `cmdParts`, write the generated config alongside the launch script. If `mkdirSync` is not yet imported at the top of `pi-extension/subagents/index.ts`, add it to the existing `node:fs` import alongside `writeFileSync`.
...
```ts
      ...
      // The artifactDir is computed lazily by `getArtifactDir()` and is not
      // guaranteed to exist yet on the first launch — `mkdirSync` it before
      // writing so the fallback never fails on a missing parent dir.
