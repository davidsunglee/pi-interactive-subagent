...
Run: `npm install`
Expected: `npm ls @modelcontextprotocol/sdk` now reports a concrete version under the package, no errors. Do NOT run `npm run build:plugin` yet — `tsc -p pi-extension/subagents/plugin` errors with "no inputs found" until Task 2 adds `mcp/server.ts`. The first plugin build verification is Task 2 Step 5.

- [ ] **Step 6: Commit**

```bash
git add package.json package-lock.json pi-extension/subagents/plugin/tsconfig.json
...
## Task 8: Watcher fallback — read transcript JSONL last assistant message + bounded transcript-pointer wait (TDD)
...
This task also adds a bounded wait for `${PI_CLAUDE_SENTINEL}.transcript` before transcript archival to avoid the one-turn race where the MCP sentinel is written before the Stop hook surfaces the transcript pointer. The bounded wait is exercised end-to-end by the autonomous-with-MCP integration tests in Tasks 9 and 11, where `transcriptPath` and `sessionId` must be populated even when the model completes in a single turn.
...
- [ ] **Step 5: Wire the helper into `watchSubagent`'s fallback chain and add the bounded transcript-pointer wait**
...
This is the first live exercise of plugin-MCP auto-discovery. Confirm the model could actually invoke `mcp__pi-subagent__subagent_done` before continuing. The temp `env.dir` is cleaned up by `cleanupTestEnv` and the watcher unlinks the sentinel + pointer files after finalization, so they cannot be inspected post-hoc — read the durable archived transcript via `result.transcriptPath` instead.

Add an in-test assertion that pins the auto-load behavior to a durable artifact. Edit `test/integration/claude-sentinel-roundtrip.test.ts`, after the existing `result.summary` assertion:
...
```ts
      ...
      // discovery is unreliable on this Claude version and Task 10's
      // --mcp-config fallback must be implemented before proceeding.
        ...
        .some((line) => {
          try {
            ...
            return Array.isArray(blocks) && blocks.some(
              (b: any) => b?.type === "tool_use" && b?.name === "subagent_done",
            );
          } catch { return false; }
        });
      assert.ok(
...
(Add `import { readFileSync } from "node:fs";` to the test file if it is not already imported.)

If `claude` is on PATH locally:
- Run: `npm run build:plugin && npm run test:integration -- --test-name-pattern=claude-sentinel-roundtrip`
- The test now self-verifies plugin-MCP auto-load via the durable archived transcript at `result.transcriptPath` (under `~/.pi/agent/sessions/claude-code/...`); no manual post-hoc inspection is required.

Decide:
- **Auto-load worked** (test passes): SKIP Task 10. Continue with Task 11.
- **Auto-load failed** (assertion above fires, OR the watcher returns with `error: "cancelled"` after timeout, OR the transcript shows an "unknown tool" error for `subagent_done`): PROCEED to Task 10 to add the `--mcp-config` fallback before continuing.
...
In `launchSubagent`'s Claude branch, immediately before the `buildClaudeCmdParts({ ... })` call (around index.ts:676), write the generated config inside the session artifact directory using the variables that are actually in scope: `spec.artifactDir`, `params.name` (sanitized via the existing `safeScriptName` helper), `id` (the launch id), and `pluginDir` (the absolute resolved plugin dir local). If `mkdirSync` is not yet imported at the top of `pi-extension/subagents/index.ts`, add it to the existing `node:fs` import alongside `writeFileSync`.
...
```ts
      ...
      //
      // The artifact subdir is created here (not assumed to exist) — the
      // launch-script writer below uses the same pattern. `safeScriptName` is
      // the existing helper used a few lines below for the launch script
      // filename; reuse it so both files have matching sanitization.
      const mcpConfigDir = join(spec.artifactDir, "mcp-configs");
      mkdirSync(mcpConfigDir, { recursive: true });
      const mcpConfigPath = join(
        mcpConfigDir,
        `${safeScriptName(params.name, "subagent")}-${id}.mcp.json`,
      );
      writeFileSync(
        mcpConfigPath,
        JSON.stringify({
...
Then pass `mcpConfigPath` into the existing `buildClaudeCmdParts({ ..., mcpConfigPath })` call.
...
import { launchSubagent, watchSubagent, extractLastAssistantMessage } from "../../pi-extension/subagents/index.ts";
...
for (const backend of backends) {
  describe(`pane-claude-interactive [${backend}]`, { skip: SHOULD_SKIP, timeout: PI_TIMEOUT * 2 }, () => {
    ...
    it("empty-message: watcher falls back to transcript JSONL last assistant message (Finding 3 wiring)", async () => {
      ...
      //
      // The model emits a unique probe string as its final assistant message,
      // then calls subagent_done with NO message argument so the sentinel is
      // empty. We then read the archived transcript ourselves with the same
      // helper the watcher uses and assert result.summary matches byte-for-byte.
      // A screen scrape would include surrounding pane chrome (prompt, user
      // input echo, etc.) and would not match exactly.
      ...
      const running = await launchSubagent(
        {
          ...
          agent: "test-claude-autoexit",
          task:
            `Reply with EXACTLY this line and nothing else: ${PROBE}\n` +
      ...
      assert.match(
        result.summary,
        new RegExp(PROBE),
        `expected summary to contain probe ${PROBE}; got: ${result.summary}`,
      );
      const fromTranscript = extractLastAssistantMessage(
...
- [ ] **Step 3: Build the plugin and run the new test under the slow-lane gate**

Run: `npm run build:plugin && PI_RUN_SLOW=1 node --test test/integration/pane-claude-interactive.test.ts`
Expected: tests run (not skip) when `claude` is on PATH and a mux backend is present. The `SLOW_LANE_OPT_IN` skip gate is `process.env.PI_RUN_SLOW === "1"`, so the prefix is required for this step to actually exercise the new behavior. If `claude` or the mux backend is not available, the suite still skips even with `PI_RUN_SLOW=1` — inspect the skip log line to confirm the gate fired for the right reason.
...
- [ ] **Step 2: Run the serial test under the slow-lane gate**

Run: `npm run build:plugin && PI_RUN_SLOW=1 node --test test/integration/orchestration-claude-pane-serial.test.ts`
Expected: skipped if `claude` or a mux backend is not available; otherwise passes once Tasks 1–11 land. The `PI_RUN_SLOW=1` prefix is required so the `SLOW_LANE_OPT_IN` skip gate flips on. If you run this in isolation before Tasks 1–11, the test launches but the model has no `subagent_done` and the pane hangs — failing as expected for a TDD red.
...
for (const backend of backends) {
  describe(`orchestration-claude-pane-parallel [${backend}]`, { skip: SHOULD_SKIP, timeout: PI_TIMEOUT * 3 }, () => {
    ...
    it("runParallel with two cli=claude tasks: each child gets its own sessionKey + payload", async () => {
      ...
      // its `CLARIFY-<name>?` marker, then send the matching reply.
      const driveOne = async (childName: "alpha" | "beta") => {
        ...
        env.surfaces.push(surface);
        await waitForScreen(surface, new RegExp(`CLARIFY-${childName}\\?`), 30_000);
...
- [ ] **Step 4: Run the parallel test under the slow-lane gate**

Run: `npm run build:plugin && PI_RUN_SLOW=1 node --test test/integration/orchestration-claude-pane-parallel.test.ts`
Expected: skipped without Claude or a mux backend; passes with both available. The `PI_RUN_SLOW=1` prefix is required so the `SLOW_LANE_OPT_IN` skip gate flips on.
...
for (const backend of backends) {
  describe(`orchestration-claude-pane-spec-designer-e2e [${backend}]`, { skip: SHOULD_SKIP, timeout: PI_TIMEOUT * 4 }, () => {
    ...
    it("dispatches through runSerial; observes Q1 then Q2; parent payload carries SPEC_WRITTEN", async () => {
      ...
      // the parent-facing contract is `SPEC_WRITTEN: <abs path>`, so the test
      ...
      // subagent_done with the SPEC_WRITTEN message anyway, the driver's
      ...
      // workflow contract is `SPEC_WRITTEN: <abs path>` — assert both the
...
- [ ] **Step 3: Build and run under the slow-lane gate**

Run: `npm run build:plugin && PI_RUN_SLOW=1 node --test test/integration/orchestration-claude-pane-spec-designer-e2e.test.ts`
Expected: PASS when Claude + a mux backend are available; skips otherwise. The `PI_RUN_SLOW=1` prefix is required so the `SLOW_LANE_OPT_IN` skip gate flips on.
...
- [ ] **Step 1: Build the plugin (refresh `mcp/server.js`)**

The plugin's compiled `server.js` is committed to keep end-user installs zero-build, but the root `tsconfig.json` excludes `pi-extension/subagents/plugin/**` so `npm run typecheck` does not refresh it. Always rebuild before running tests so a stale `server.js` cannot pass while `server.ts` is broken.

Run: `npm run build:plugin`
Expected: exit 0; `pi-extension/subagents/plugin/mcp/server.js` mtime is fresh.
...
- [ ] **Step 4: Run the default integration suite (expect skips on CI without claude / SLOW_LANE_OPT_IN)**

Run: `npm run test:integration`
Expected: passes; the new heavy Claude pane suites (Tasks 11–13) skip without `PI_RUN_SLOW=1`.

- [ ] **Step 5: Run the slow lane (Tasks 11–13 unskipped) locally**

Run: `PI_RUN_SLOW=1 npm run test:integration`
Expected: passes locally where `claude` and a mux backend are available; the new pane / orchestration / spec-designer tests now execute (their `SLOW_LANE_OPT_IN` gate flips on under `PI_RUN_SLOW=1`). Gracefully skips on machines without Claude or a mux backend.

- [ ] **Step 6: Source-TODO closure (optional, requires user approval)**
...
- Task 13 additionally asserts `finalMessage.includes(SPEC_PATH)` so the parent-facing `SPEC_WRITTEN: <abs path>` workflow contract is pinned, not just the `SPEC_WRITTEN:` prefix.

---

## Execution Handoff

Plan complete and saved to `docs/superpowers/plans/2026-04-26-claude-pane-interactive-subagents-v6.md`. Two execution options:
