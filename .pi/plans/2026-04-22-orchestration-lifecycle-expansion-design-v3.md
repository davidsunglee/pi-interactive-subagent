...
- **Ownership key (`sessionKey`):** one canonical identifier is threaded end-to-end. For pi-backed children, `sessionKey = subagentSessionFile` (a file path — identical to what `subagent_resume({ sessionPath })` accepts). For Claude-backed children, `sessionKey = claudeSessionId`. The registry's ownership map, the blocked notification's `sessionKey` payload field, and the `subagent_resume` lookup all use this same value. Because `subagent_resume` today takes only `sessionPath`, Claude-session-id-keyed re-ingestion is a documented v1 gap; pi-backed children are the fully-supported path.
...
- `test/integration/orchestration-pane-block-backend.test.ts` (Phase 2) — **backend-real** pane async/block test: no `LauncherDeps` or `watchSubagent` seam; `test-ping` child actually pings and the pane adapter's `BackendResult.ping` / `sessionKey` propagation (Task 9.4) is exercised end-to-end.
- `test/integration/orchestration-headless-block-backend.test.ts` (Phase 2) — **backend-real** headless async/block test: exercises `runPiHeadless`'s `.exit` sidecar detection (Task 9.5) end-to-end, asserting the real session file path surfaces in the `blocked` steer-back and the sidecar is cleaned up on resume.
...
- `pi-extension/subagents/backends/types.ts` — add `ping?: { name: string; message: string }` to `BackendResult`; add `sessionKey?: string` (the stable identifier parent uses with `subagent_resume`: session file path for pi children, Claude session id for Claude children).
- `pi-extension/subagents/backends/pane.ts` — propagate `sub.ping` into `BackendResult.ping`; populate `BackendResult.sessionKey` from `running.sessionFile` for pi children and from `sub.claudeSessionId` for Claude children.
- `pi-extension/subagents/backends/headless.ts` (Phase 2) — detect `${subagentSessionFile}.exit` ping sidecar on child close and set `BackendResult.ping` accordingly; populate `BackendResult.sessionKey` from `spec.subagentSessionFile` for pi and from the Claude init event for Claude.
- `pi-extension/subagents/index.ts` — register orchestration tools with `pi` handle wired for steer-back emission; extend widget to render `blocked` state keyed on `(orchestrationId, taskIndex)`; in `subagent_resume.execute`, route both the terminal result and any ping-during-resume into the registry (via `onResumeTerminal` for terminal, `onTaskBlocked` for re-ping) so cross-orch re-ingestion and recursion work through the real resume tool.
...
- The registry ownership map is keyed on the same value that `subagent_resume({ sessionPath })` accepts — the session file path for pi-backed children. Blocked notifications carry this same value in the `sessionKey` payload field, matching the spec.
...
- `registry.onResumeTerminal` invokes the continuation callback **only when the resumed slot's `state === "completed"`**. A failed or cancelled resume must not advance the serial pipeline. Regression tests exist for both `failed` and `cancelled` resume outcomes.
...
Also extend the existing `LaunchedHandle` type in the same file with an optional `sessionKey` field. This is the stable resume-addressable identifier that the backends populate at launch time — the session file path for pi-backed children (matches `subagent_resume({ sessionPath })`), the Claude session id for Claude-backed children. It is threaded through to the registry's ownership map.
...
describe("async orchestration — real subagentsExtension wiring", () => {
  it("extension registers subagent_run_{serial,parallel,cancel} and delivers orchestration_complete via pi.sendMessage", async () => {
    ...
    // 2. Override LauncherDeps at the extension's seam (or use the existing
    //    deps-injection test hook) with fast deterministic children.
    // 3. Invoke subagent_run_serial with wait: false. Assert the handler
    //    returns an envelope with `orchestrationId` and `state: "pending"`
    //    per task.
    // 4. Await background completion. Assert exactly one
    //    pi.sendMessage({ customType: "orchestration_complete", details: ... },
    //      { deliverAs: "steer", triggerTurn: true }) was emitted.
    // 5. Assert `details.results` carries the terminal per-task states.
    //
...
- [ ] **Step 9.4: Surface ping + sessionKey in the pane adapter**

In `pi-extension/subagents/backends/pane.ts::watch`, extend the return to carry `sub.ping` and to populate `sessionKey` explicitly. For pi-backed children, `sessionKey` is the session file path on the `RunningSubagent` (the pane adapter already has access to `running` at this point — if not, thread it in). For Claude-backed children, set `sessionKey` to `sub.claudeSessionId` so the same field is always populated, even though Claude children are not currently resumable through `subagent_resume({ sessionPath })`.
...
return {
  ...
  sessionKey: running.sessionFile ?? sub.claudeSessionId, // NEW
  error: sub.error,
  ping: sub.ping, // NEW
};
```
...
Also in `headless.ts::launch`, populate `LaunchedHandle.sessionKey: spec.subagentSessionFile` for pi. For Claude-backed headless runs, leave `sessionKey` unset at launch time — the registry can accept a late update if the backend learns the id after launch, but the v1 plan documents this as a gap rather than implementing late-binding.

- [ ] **Step 9.6: Propagate ping + sessionKey through makeDefaultDeps**

In `pi-extension/orchestration/default-deps.ts::waitForCompletion`, add both new fields to the final projection:
...
Also add `ping` and `sessionKey` to the `OrchestrationResult` interface in `types.ts`:
...
describe("async orchestration — blocked notification through the real extension", () => {
  it("a pinging child surfaces exactly one customType:'blocked' sendMessage with spec-shaped details", async () => {
    ...
    // 2. Inject deps whose waitForCompletion for the first task returns
    //    { ..., sessionKey: "/tmp/owned.jsonl", ping: { name, message } }.
    // 3. Invoke subagent_run_serial with wait: false.
    // 4. Assert:
    //    - customType === "blocked"
    //    - details.kind === "blocked"
    //    - details.orchestrationId matches the envelope
    //    - details.sessionKey === "/tmp/owned.jsonl"
    //    - details.message === <injected ping message>
    //    - sendMessage options include { deliverAs: "steer", triggerTurn: true }
    // 5. Assert the widget has a virtual blocked row (via __test__ surface).
  });
});
```
...
Store the callback on the `OrchestrationEntry`. Refine `onResumeTerminal` (introduced in Task 3) to invoke the continuation **only when the resumed slot was blocked AND the resumed result is a successful completion** (`state === "completed"`). A resumed slot that lands `failed` or `cancelled` must NOT advance the serial pipeline — downstream tasks stay `pending` and the aggregated completion reports the failed/cancelled slot.
...
onResumeTerminal(sessionKey, result) {
  ...
  this.onTaskTerminal(own.orchestrationId, own.taskIndex, result);
  // Only advance the serial continuation when the resumed slot
  // completed successfully. A failed/cancelled resume is terminal for
  // the orchestration's forward progress; the aggregated-completion
  // path in onTaskTerminal will emit the final notification with the
  // failed/cancelled slot plus any untouched `pending` tasks surfaced
  // as `cancelled` by the sweep the registry performs when no tasks
  // remain runnable.
...
Invariant: if the resumed slot is `failed` or `cancelled`, the continuation callback is not invoked. Any `pending` downstream tasks remain un-launched; the registry's existing aggregate-completion logic (which fires when every task is terminal) is responsible for converting those `pending` tasks into `cancelled` in the aggregated result so the orchestration can finalize. Implementations that rely on a per-orchestration "no more work" sweep already do this in Task 6 (`subagent_run_cancel`); extend that sweep path so it also runs when a blocked-and-resumed slot lands non-successfully and the orchestration has no other in-flight work.
...
**Ownership key contract recap.** The registry's ownership map is keyed on the same value the parent will pass back through `subagent_resume({ sessionPath })`. For pi-backed children, that value is the session file path (set at launch time from `running.sessionFile` / `spec.subagentSessionFile` and threaded through `BackendResult.sessionKey` → `LaunchedHandle.sessionKey` → `registry.onTaskLaunched({ sessionKey })`). Claude-backed children set `sessionKey = claudeSessionId`; they are a documented v1 gap because `subagent_resume` does not accept Claude session ids today.
...
const sessionKey = params.sessionPath; // matches the pi-backed ownership key
...
The tests above prove the registry/tool-handler bridge with injected `LauncherDeps` seams. This test proves that an actual pane backend result carrying a ping is transformed all the way into the expected `blocked` / completion behavior — without injecting `LauncherDeps` or seaming `watchSubagent`. It closes the gap review finding #2 flagged: the pane backend's `ping` / `sessionKey` propagation in `pi-extension/subagents/backends/pane.ts` (from Task 9.4) is currently only covered indirectly.

Use the existing `test-ping.md` agent under `test/integration/agents/` and the pi-pane-smoke harness pattern for backend selection / skip semantics. The test must:

1. Boot the real `subagentsExtension(pi)` against a capturing fake `pi` (same pattern as Step 10.7b).
2. Do NOT inject `LauncherDeps` — let the extension use its real `makeDefaultDeps` so that `launchSubagent` → `watchSubagent` → `pane.ts` exercise the genuine backend path.
3. Dispatch `subagent_run_serial` with `wait: false` and one task that uses the `test-ping` agent so the child calls `caller_ping` in practice.
4. Assert one `customType: "blocked"` `pi.sendMessage(...)` fires with `details.sessionKey` equal to the subagent session file path the pane backend actually used (not a fabricated one), and `details.message` equal to the ping message emitted by the agent.
5. Invoke the registered `subagent_resume` tool with that `sessionPath`; have the agent terminate cleanly on resume.
...
for (const backend of backends) {
  describe(`orchestration-pane-block-backend [${backend}]`, {
    ...
    it("pane: caller_ping from a real child → blocked steer-back → subagent_resume → orchestration_complete", async () => {
      ...
      // pi-pane-smoke test as a reference for the minimum surface.
      // Boot subagentsExtension(pi). Dispatch subagent_run_serial with a
      // single task referencing agents/test-ping.md (which calls caller_ping).
      // Await the BLOCKED_KIND sendMessage; assert sessionKey is the pane's
      // actual session file path. Then invoke the subagent_resume tool with
      // that sessionPath and a follow-up prompt that makes the agent finish.
      // Await the ORCHESTRATION_COMPLETE_KIND sendMessage and assert the
...
The headless path does not require mux/pane availability, so this test runs whenever `pi` is on PATH. Use the same skip sentinel as `headless-pi-smoke.test.ts`.

The test must:

1. Boot the real `subagentsExtension(pi)` against a capturing fake `pi`.
2. Configure `makeDefaultDeps` for the headless backend (force `PI_SUBAGENT_BACKEND=headless` or equivalent harness switch — check `harness.ts`; if a helper doesn't exist, set the env var directly before booting the extension and restore in `after`).
3. Dispatch `subagent_run_serial` with `wait: false` and one task referencing `test-ping`.
4. Assert a `BLOCKED_KIND` `pi.sendMessage(...)` fires with `details.sessionKey` equal to `spec.subagentSessionFile` the headless runner actually created (use `existsSync` or the seeded session dir to verify the path is real).
5. Call the registered `subagent_resume` tool against that `sessionPath` with a follow-up prompt that lets the agent terminate normally.
6. Assert `ORCHESTRATION_COMPLETE_KIND` is emitted with `state: "completed"` and that `${sessionFile}.exit` no longer exists after resume (sidecar cleanup invariant from Task 9.5).
...
import { execSync } from "node:child_process";
import { createTestEnv, cleanupTestEnv, PI_TIMEOUT, type TestEnv } from "./harness.ts";
import { subagentsExtension } from "../../pi-extension/subagents/index.ts";
import {
  BLOCKED_KIND, ORCHESTRATION_COMPLETE_KIND,
} from "../../pi-extension/orchestration/notification-kinds.ts";
...
describe("orchestration-headless-block-backend", {
  ...
  let env: TestEnv;
  before(() => {
    prevBackend = process.env.PI_SUBAGENT_BACKEND;
    process.env.PI_SUBAGENT_BACKEND = "headless";
    env = createTestEnv("headless");
  });
  after(() => {
    cleanupTestEnv(env);
    if (prevBackend === undefined) delete process.env.PI_SUBAGENT_BACKEND;
    else process.env.PI_SUBAGENT_BACKEND = prevBackend;
  });

  it("headless: caller_ping from a real child → blocked steer-back → subagent_resume → orchestration_complete", async () => {
    // Capturing fake pi (ExtensionAPI shape). Boot subagentsExtension(pi).
    // Dispatch subagent_run_serial({ wait: false, tasks: [{ agent: "test-ping", task: "ping me" }] }).
    // Await BLOCKED_KIND; assert details.sessionKey is a real file path that
    // existsSync()'s. Assert `${sessionKey}.exit` was removed by the backend
    // after propagation (Task 9.5 invariant).
    // Invoke subagent_resume with that sessionPath; assert ORCHESTRATION_COMPLETE_KIND
    // fires with state: "completed".
  });
});
```

Run: `node --test test/integration/orchestration-headless-block-backend.test.ts`
Expected: PASS on machines where `pi` is available; skipped otherwise.
...
1. Orchestration emits a `blocked` steer-back with `{ orchestrationId, taskIndex, taskName, sessionKey, message }`, where `sessionKey` is the same value the parent passes to `subagent_resume({ sessionPath })` (the session file path for pi-backed children).
...
3. Parent resumes the child via standalone `subagent_resume({ sessionPath: <sessionKey from blocked event> })`; on terminal completion, the result is re-ingested into the original orchestration. If the resumed child pings again, the orchestration re-blocks (recursive unblock loop supported).
...
- Claude sessions: `subagent_resume` only accepts session file paths, so Claude-session-id-keyed ownership cannot currently route back into the owning orchestration. Pi-backed children are the fully-supported path.
...
```bash
git add README.md test/integration/orchestration-block-resume-e2e.test.ts test/integration/orchestration-extension-resume-routing.test.ts test/integration/orchestration-pane-block-backend.test.ts test/integration/orchestration-headless-block-backend.test.ts
...
- Tool renames → Task 4
- Session-ownership map → Task 3, 9, 12
- caller_ping detection → Task 9
- Blocked steer-back (`kind: "blocked"`) → Tasks 10, 11
- Resume re-ingestion (terminal + ping-during-resume) → Tasks 11–12
- Recursion through real `subagent_resume` → Task 12, 14
- Cancellation of blocked → Task 10 (registry test)
- Widget blocked state (clears on per-task terminal) → Task 13
- Real-extension integration coverage → Tasks 8.3b, 10.7b, 14.2
- Backend-real async/block coverage (pane + headless) → Tasks 14.2b, 14.2c
