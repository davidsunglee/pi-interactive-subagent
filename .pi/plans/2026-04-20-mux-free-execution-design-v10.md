# Mux-Free Execution Implementation Plan (v10)
...
Run: `npm test 2>&1 | tail -40`
Expected: every unit test passes, including `buildClaudeHeadlessArgs` (5 cases), the existing `parseClaudeStreamEvent` / `parseClaudeResult` cases, and the three `PI_TO_CLAUDE_TOOLS` cases from Step 2b.
...
git commit -m "feat(backends): headless Claude consumes ResolvedLaunchSpec; thread --resume; exit-tracked SIGKILL; session-id transcript discovery; lock shared tool-map coverage"
```
