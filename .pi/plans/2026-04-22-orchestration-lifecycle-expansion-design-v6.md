...
- The registry ownership map is keyed on the canonical `sessionKey` value: the session file path for pi-backed children (same value accepted by `subagent_resume({ sessionPath })`) and the Claude session id for Claude-backed children (same value accepted by `subagent_resume({ sessionId })`). Blocked notifications carry this value in the `sessionKey` payload field, matching the spec. Claude children's `sessionKey` is late-bound via `registry.updateSessionKey` from the `system/init` event (Step 9.5b); pi children bind at launch.
...
- Modify: `pi-extension/orchestration/types.ts` (extend `LauncherDeps.waitForCompletion` with `hooks?.onSessionKey` — Step 9.5b)
- Modify: `pi-extension/orchestration/run-serial.ts`, `run-parallel.ts` (accept `onSessionKey` opt and forward — Step 9.5b)
- Modify: `pi-extension/orchestration/tool-handlers.ts` (wire `onSessionKey → registry.updateSessionKey` in the async branches — Step 9.5b)
...
In `pi-extension/subagents/backends/pane.ts::watch`, extend the return to carry `sub.ping` and to populate `sessionKey` explicitly. For pi-backed children, `sessionKey` is the session file path on the `RunningSubagent` (the pane adapter already has access to `running` at this point — if not, thread it in). For Claude-backed children, set `sessionKey` to `sub.claudeSessionId` so the same field is always populated; `subagent_resume({ sessionId })` re-opens these sessions (Task 12). If `sub.claudeSessionId` becomes known mid-run before the pane result has been returned, also invoke the `hooks?.onSessionKey(sub.claudeSessionId)` callback threaded through `waitForCompletion` (Step 9.5b) so the registry's ownership map is populated before any mid-run `caller_ping` event rather than only at result time.
...
- [ ] **Step 9.5b: Late-bind Claude `sessionKey` through a mid-run seam**

The pi case surfaces `sessionKey` at launch time because the session file path is provisioned before the child starts. The Claude case only learns the id after the child has been running for some milliseconds, so we need a mid-run callback that flows from the backend → `LauncherDeps` → `run-serial`/`run-parallel` → the async dispatcher → `registry.updateSessionKey`.

Extend `LauncherDeps` (in `pi-extension/orchestration/types.ts`) with an optional `onSessionKey` callback on `waitForCompletion`:
...
`default-deps.ts::waitForCompletion` threads the `onSessionKey` hook down into the backend:

- **Pane backend (`pane.ts::watch`):** for Claude children, call `hooks.onSessionKey?.(sub.claudeSessionId)` the first time `sub.claudeSessionId` becomes known inside `watchSubagent` (i.e., add the invocation where the existing streaming path records the Claude session id, not at the end). Pi children already propagate `sessionKey` at launch so this hook is a no-op for them.
- **Headless Claude (`runClaudeHeadless`):** invoke `hooks.onSessionKey?.(claudeSessionId)` the first time the `system/init` event is parsed. The existing code already has the `system/init` handler — add the callback there. For `runPiHeadless`, no mid-run binding is needed (the session file path is already set at launch).

Extend `run-serial.ts` / `run-parallel.ts` to accept an `onSessionKey` option in `RunSerialOpts` / `RunParallelOpts` and forward it to `deps.waitForCompletion`:
...
Wire the option in `tool-handlers.ts` async branches (alongside the existing `onLaunched`/`onTerminal`/`onBlocked` wiring):

```ts
onSessionKey: (i, sessionKey) => registry.updateSessionKey(orchestrationId, i, sessionKey),
```

Add a unit test in `test/orchestration/registry.test.ts` (next to the Step 3.1 `updateSessionKey` tests) that simulates a Claude-backed launch: `onTaskLaunched` with no `sessionKey`, then `updateSessionKey` with the Claude id, then `onTaskBlocked` on that id. Assert the blocked steer-back fires with `sessionKey` set to the Claude id. (Already covered by the "a late-bound sessionKey routes subsequent onTaskBlocked / onResumeTerminal via ownership" test added in Step 3.1 — verify this test exists and that Task 9.5's wiring makes it reachable from the runner layer.)
...
- Session-ownership map (pi path + Claude path) → Task 3 (registry + `updateSessionKey`), Task 9 (backend + Step 9.5b late-bind), Task 12 (resume routing for both `sessionPath` and `sessionId`)
...
- Claude-backed `sessionKey` late-binding from `system/init` → Step 9.5b
...
- Claude-backed block/resume coverage (registry level) → Step 12.1 Claude test; Step 3.1 `updateSessionKey` tests
