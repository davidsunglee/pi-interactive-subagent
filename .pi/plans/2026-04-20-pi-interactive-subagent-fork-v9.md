# pi-interactive-subagent Fork Implementation Plan (v9)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

> **v9 revision notes** (addresses `.pi/plans/reviews/2026-04-20-pi-interactive-subagent-fork-review-v8.md`):
> 1. **Top-of-file revision history no longer contradicts the corrected v8 Task 18 body.** v8 rewrote Task 18 Step 3 to describe the bundled-plugin degraded-mode story accurately (missing `plugin/` subtree → `launchSubagent` omits `--plugin-dir`; pane-screen `__SUBAGENT_DONE_<code>__` sentinel still drives `pollForExit`; what's lost is `transcriptPath` / `claudeSessionId` / structured-summary quality; ending the pi session is the reliable interrupt lever). But two earlier carried-forward bullets still described superseded wordings: v7 note #2 claimed "kill the Claude pane directly via the mux" was a supported recovery path (it is not — `pollForExit` only catches a pi-branch `<sessionFile>.exit` sidecar, which Claude runs do not produce, so killing the pane alone leaves no clean completion signal), and "Carried from v4" note #3 still summarized the old "sentinel wait hangs until external timeout/cancel fires" rewrite that v8 replaced. The stale v7 note is dropped and the v4-carried note's missing-plugin sub-claim is rewritten to point at the current (v8) degraded-mode story. A retroactive **v8 revision notes** block has also been added — v8 made real fixes but skipped its own history entry.
> 2. **Document title corrected.** The first heading said `(v7)` even though the file was `-v8.md`. The v9 title now matches the filename, and this revision's edits are recorded under v9.
>
> **v8 revision notes** (retroactively documented; addresses `.pi/plans/reviews/2026-04-20-pi-interactive-subagent-fork-review-v7.md`):
> 1. **README now matches the actual bundled-plugin model.** v7's README copy still implied a `claude plugin install` step. The fork ships the Stop hook inside the repo at `pi-extension/subagents/plugin/` and `launchSubagent` auto-loads it via `--plugin-dir` (see `buildClaudeCmdParts` wiring in Task 6). Task 18 Step 3 was rewritten to say so explicitly, tell users to remove any stale symlink under `~/.claude/plugins/`, and describe what the bundled plugin actually provides (clean completion signal → `summary`, `claudeSessionId`, archived `transcriptPath`).
> 2. **Missing-plugin degraded mode is described correctly.** v7 implied completion detection was plugin-dependent. In reality `launchSubagent` appends `; echo '__SUBAGENT_DONE_'$?'__'` to every Claude command and `pollForExit` scans the pane screen for `__SUBAGENT_DONE_(\d+)__`, so completion is still detected without the plugin. What degrades is the archived transcript under `~/.pi/agent/sessions/claude-code/`, `SubagentResult.transcriptPath` (falls back to `null`), `SubagentResult.claudeSessionId`, and summary quality (screen-scraped tail instead of Stop-hook structured final message). Task 18 Step 3 was rewritten to enumerate these losses explicitly.
> 3. **`default-deps.test.ts` rationale is narrowed honestly.** v7 implied the test verified the real `watchSubagent → OrchestrationResult` mapping. It doesn't — it cannot without standing up a pane. Task 13 Step 2's "Scope caveat" paragraph now states that the test only covers the unknown-handle branch of `waitForCompletion` and that a stubbed-`../subagents/index.ts` follow-up is a reasonable additive next step but is out of scope here.
>
> **Carried from v7** (still true — addresses `.pi/plans/reviews/2026-04-20-pi-interactive-subagent-fork-review-v6.md`):
> 1. **`OrchestrationTaskSchema` now restores full parity with the already-supported `SubagentParams` surface.** v6 omitted five fields that `launchSubagent()` already accepts today (`systemPrompt`, `skills`, `tools`, `fork`, `resumeSessionId`), silently reducing the orchestration API surface relative to the bare `subagent` tool while claiming the wrappers were "thin" over it. Task 8 adds these fields to `OrchestrationTaskSchema`, Task 13 forwards them through `makeDefaultDeps.launch` into `launchSubagent`, and Task 18 Step 2's parameter reference is extended to list them alongside `cli` / `thinking` / `focus`. The in-file comment is rewritten to name only the truly-unplumbed fields (`interactive`, `permissionMode`) so future readers are not misled.
> 2. **`cli` and `thinking` validation stance is now explicit.** v6 documented these fields as having a constrained value set but declared the schema fields as free-form `Type.String()`, so typos pass through silently. Rather than tighten to enums mid-plan (which would change agent-frontmatter parsing semantics too), the plan now documents the current fallback behavior at the schema boundary (Task 4 Step 1 + Task 8 Step 1) and lists enum-tightening as a Deferred-work item.
>
> *(v7 note #2, which described a now-superseded rewrite of Task 18 Step 3's missing-plugin wording, is dropped — the v8 revision notes above document the current, accurate wording.)*
>
> **Carried from v6** (still true — addresses `.pi/plans/reviews/2026-04-20-pi-interactive-subagent-fork-review-v5.md`):
...
> - `OrchestrationTaskSchema` omits only `interactive` / `permissionMode` — the two fields that `launchSubagent()` does not accept today. All other `SubagentParams` fields are plumbed through (v7 fix).
...
- `README.md` — rewrite title + install URL + "What's Included" copy, add new tool section, fork provenance note, bundled-Claude-plugin note (plugin ships in the repo and is auto-loaded via `--plugin-dir`; no manual install).
...
**Validation note.** Both `cli` and `thinking` are declared as free-form `Type.String()` here (and in `OrchestrationTaskSchema` in Task 8) rather than typebox `Type.Union([Type.Literal(...), ...])` enums. This is intentional for v1: the existing agent-frontmatter parser accepts the same values as free-form strings, so tightening the schema here without also tightening frontmatter parsing would create two different validators for the same field. The runtime fallback for unknown values today is:

- **`cli`:** any value that is not literally `"claude"` falls through to the pi path (see Task 4 Step 3 — the resolution uses `=== "claude"` as the only positive branch).
- **`thinking`:** on Claude, `thinkingToEffort` returns `undefined` for unknown values, so `--effort` is simply omitted; on pi, the raw value is appended to the model string as `<model>:<thinking>` without validation.

Typos therefore pass silently rather than error. This is survivable for the rebrand; enum-tightening across both `SubagentParams` and the frontmatter parser is listed under Deferred work.
...
Goal: prove that the **unknown-handle branch** of `makeDefaultDeps.waitForCompletion` returns `transcriptPath: null` (not `undefined`) on the `OrchestrationResult`. This is a narrow shape test — it locks the synthetic "no running entry for <id>" error return down to the same `transcriptPath: null` contract Task 7 Step 3's catch block enforces on the real `watchSubagent` path, so a future optional-field relaxation in this file cannot silently let `undefined` leak through.

**Scope caveat (honest, not aspirational):** this test does **not** drive real `launchSubagent` / `watchSubagent` — it cannot, without standing up a pane — so it does **not** directly verify the real `watchSubagent(...) → OrchestrationResult` mapping that Task 7 is changing. A follow-up test that stubs `../subagents/index.ts` to return a synthetic failure `SubagentResult` and asserts the same `transcriptPath: null` contract on the real mapping branch is a reasonable additive next step, but is out of scope here. The unknown-handle check is sufficient to guard the contract on the one code path this file owns end-to-end.
...
## Task 18: README updates — intro correction + tool docs + existing-section touch-ups + bundled-plugin note + smoke checklist
...
3. **Parameter / agent-frontmatter reference.** The README has two distinct reference surfaces — the per-call **tool parameter** reference and the **agent-frontmatter** reference. These three new fields split across them differently, so do not fold them into a single table:
   ...
   **Tool parameters only** (all three are per-call `SubagentParams` fields):
   - `cli` — `'pi' | 'claude'`, overrides agent frontmatter. Free-form string; unknown values fall back silently to the pi path (see the Deferred-work entry on enum tightening).
   - `thinking` — `off | minimal | low | medium | high | xhigh`; pi folds into `<model>:<thinking>`, Claude maps via `thinkingToEffort` to `--effort`. Free-form string; unknown values are dropped on Claude (`thinkingToEffort` returns `undefined`) and pass through as a pi model suffix (see the Deferred-work entry on enum tightening).
...
4. **Orchestration task-schema parity note.** Each entry in `subagent_serial.tasks` / `subagent_parallel.tasks` accepts the full per-call `SubagentParams` surface the bare `subagent` tool already exposes (`agent`, `task`, `name`, `cli`, `model`, `thinking`, `systemPrompt`, `skills`, `tools`, `cwd`, `fork`, `resumeSessionId`), plus the new `focus` override. The only `SubagentParams`-adjacent fields not plumbed today are `interactive` and `permissionMode` (upstream `launchSubagent` doesn't accept them yet). Mention this once in the orchestration section so users don't assume the wrappers have a reduced surface.
...
```bash
git add README.md
git commit -m "docs: correct non-blocking intro, document orchestration tools, bundled Claude plugin note, smoke checklist"
```
...
- **Widget-level cancel action + tool-signal → running-wait cancellation propagation.** `runSerial` / `runParallel` accept a reserved `opts` argument and tool handlers receive an `AbortSignal`, but the current `default-deps` `waitForCompletion` creates its own local `AbortController` and does not wire the tool signal into the running subagent's wait. There is also no widget action or standalone tool that aborts an individual `RunningSubagent.abortController` — today only `session_shutdown` does. Users who need to interrupt an in-flight orchestration currently must end the pi session (see Task 18 Step 3 "Claude plugin (bundled, auto-loaded — no manual install required)" note). A future PR should add at least one of: (a) a per-subagent widget cancel action, (b) a standalone `subagent_cancel` tool, or (c) plumbing the tool signal through `runSerial`/`runParallel` → `LauncherDeps.waitForCompletion`. All three are additive.
...
- **Bundled-plugin-directory health probe / dedicated "plugin directory missing" error / bounded fallback timeout.** The Claude Stop hook ships in this repo at `pi-extension/subagents/plugin/` and is auto-loaded via `--plugin-dir` (see Task 6's `buildClaudeCmdParts` and Task 18 Step 3's "Claude plugin (bundled, auto-loaded — no manual install required)" note). If that directory is ever missing (e.g. a truncated install), the launch path silently omits `--plugin-dir` and completion falls back to screen-scraped `__SUBAGENT_DONE_(\d+)__`; callers lose archived transcripts, `transcriptPath`, and structured summaries, but completion itself is still observed. A future PR could (a) log a one-time warning when `existsSync(pluginDir)` is false, (b) surface a dedicated `SubagentResult.error` discriminant for that degraded mode, and/or (c) add a bounded fallback timeout in `pollForExit` so that even a pathological Claude that never writes `__SUBAGENT_DONE_` cannot wedge indefinitely — all additive and orthogonal to the fork rebrand + orchestration work.
...
- **Enum tightening for `cli` and `thinking`.** Both fields are free-form strings today at the tool-schema boundary and at the agent-frontmatter parser. Unknown `cli` values silently fall back to the pi path; unknown `thinking` values are dropped on Claude (`thinkingToEffort` returns `undefined`) and passed through as a pi model suffix. A future PR can swap `Type.String()` for `Type.Union([Type.Literal("pi"), Type.Literal("claude")])` (and similarly for `thinking`) and add matching validation to the frontmatter parser so typos surface as errors rather than silent fallbacks. Deferred because it changes semantics for existing agent definitions — out of scope for the rebrand.
