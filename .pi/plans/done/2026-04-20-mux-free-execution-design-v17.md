# Mux-Free Execution Implementation Plan (v17)
...
describe("headless-claude-smoke", { skip: !CLAUDE_AVAILABLE, timeout: 120_000 }, () => {
  ...
  it("runs a trivial Claude task with non-zero cost, sessionId, archived transcript", async () => {
    ...
    assert.ok(
      result.usage!.cost > 0,
      `usage.cost must be > 0 for a non-cached, freshly-run Claude task; got ${result.usage!.cost}. ` +
        `A zero cost here means cost parsing regressed: parseClaudeResult() is not reading total_cost_usd, ` +
        `or the terminal 'result' event is being dropped.`,
    );
    assert.ok(result.usage!.turns >= 1, `usage.turns must be >=1, got ${result.usage!.turns}`);
