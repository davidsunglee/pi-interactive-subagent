...
If the plugin is not installed and a `cli: "claude"` task is dispatched, the Claude process completes but no Stop-hook sentinel is ever written, so `watchSubagent` keeps polling indefinitely. This fork currently provides **no in-UI recovery path** for an individual stuck wait: there is no per-subagent cancel action on the widget, no dedicated cancel tool, and the orchestration tool's `AbortSignal` is not yet threaded into `default-deps.waitForCompletion` (see Deferred work). The only code path that actually aborts `RunningSubagent.abortController` today is `session_shutdown`, so the practical recovery options are:

1. Kill the stuck Claude pane directly via the terminal multiplexer (tmux `kill-pane`, cmux/zellij/wezterm equivalent). The pane exit does not currently cause `watchSubagent` to return — the sentinel file is still missing — so follow up with option 2.
2. End the pi session (or the surrounding tool host) and restart. `session_shutdown` fires, which aborts every running `AbortController` and clears the `runningSubagents` map, releasing the orchestration wait with an error.

This fork also does **not** currently emit a dedicated "plugin not installed" error or apply a fixed timeout — diagnosing a missing install is a manual step. A widget-level cancel action, tool-signal → running-wait propagation, auto-install, and an installation-health probe are all listed under Deferred work; until at least one lands, the wording above is the honest recovery story.
...
- **Widget-level cancel action + tool-signal → running-wait cancellation propagation.** `runSerial` / `runParallel` accept a reserved `opts` argument and tool handlers receive an `AbortSignal`, but the current `default-deps` `waitForCompletion` creates its own local `AbortController` and does not wire the tool signal into the running subagent's wait. There is also no widget action or standalone tool that aborts an individual `RunningSubagent.abortController` — today only `session_shutdown` does. Users who need to interrupt an in-flight orchestration currently must kill the pane in the mux and then end the pi session (see Task 18 Step 3 "Claude plugin install" note). A future PR should add at least one of: (a) a per-subagent widget cancel action, (b) a standalone `subagent_cancel` tool, or (c) plumbing the tool signal through `runSerial`/`runParallel` → `LauncherDeps.waitForCompletion`. All three are additive.
