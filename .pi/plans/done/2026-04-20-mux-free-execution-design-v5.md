# Mux-Free Execution Implementation Plan (v5)
...
**What's new in v5 (vs. v4):** the three blocking findings and the one non-blocking sequencing note from `.pi/plans/reviews/2026-04-20-mux-free-execution-design-review-v6.md` are remediated — see the "v5 changelog" section at the bottom of this document for the full list. In summary:
...
**Architecture (v5 — three layered changes, unchanged in shape from v2/v3/v4; the v5 refinements are (a) the backend-launch type and (b) the Claude transcript discovery helper, both called out below):**
...
- `pi-extension/subagents/index.ts` — four carried changes:
  ...
  3. **Second named patch** alongside the existing `thinking` patch: add `effectiveTools?: string` to `ClaudeCmdInputs`, route it into `buildClaudeCmdParts`, add shared `PI_TO_CLAUDE_TOOLS` constant and `--allowedTools` emission (Task 17).
...
### Task 17: Patch `buildClaudeCmdParts` with `PI_TO_CLAUDE_TOOLS` + `--allowedTools` + required `--` separator before the task
...
**v5 (review-v6 blocker 1):** this Task now also ports upstream's `--` task separator rule into the fork's `buildClaudeCmdParts()`. Claude CLI declares `--allowedTools` as a variadic option (`<tools...>`), so without a `--` separator between the last option and the task, commander can consume the prompt text as "another tool name" and the `--print` branch sees no prompt. That regression is already captured upstream: `../pi-subagent/test/claude-args.test.ts:302-316` asserts `sepIdx > toolsIdx`, `taskIdx === sepIdx + 1`, and `taskIdx === args.length - 1` (i.e. task is the very last arg, immediately after `--`). v5 adds the equivalent emit (append `"--"` after all options, then the task — but **only** when the task is non-empty — matching upstream's "omit separator when no task / empty task" behavior at `pi-subagent/test/claude-args.test.ts:319-327`) and a regression test in the same commit.
...
```ts
  ...
  it("appends task separated by -- so variadic --allowedTools does not consume it (review-v6 blocker 1)", () => {
    // Regression: Claude CLI declares --allowedTools as variadic (<tools...>).
    // If the task is pushed directly after --allowedTools, commander consumes
    ...
    const toolsIdx = parts.indexOf("--allowedTools");
    const sepIdx = parts.lastIndexOf("--");
    const taskIdx = parts.indexOf(shellEscape("Write a plan for X."));
    assert.ok(toolsIdx >= 0, "expected --allowedTools in parts");
    assert.ok(sepIdx > toolsIdx, `expected -- separator after --allowedTools (got sepIdx=${sepIdx}, toolsIdx=${toolsIdx})`);
...
Note on `shellEscape`: the existing fork builder pushes the task as `shellEscape(input.task)` (`pi-extension/subagents/index.ts:687`), so comparing `parts.indexOf(...)` against the escaped form keeps the assertion stable regardless of how `shellEscape` wraps the string (single-quoted, unchanged for trivial inputs, etc.). If `shellEscape` is not already imported in `thinking-effort.test.ts`, add it to the existing imports from `../../pi-extension/subagents/index.ts` — it is re-exported there.

- [ ] **Step 2: Run the new tests to confirm they fail**

Run: `npm test -- --test-name-pattern='buildClaudeCmdParts' 2>&1 | tail -30`
Expected: FAIL on all three new cases — `effectiveTools` is not in the `ClaudeCmdInputs` type, no `--allowedTools` is emitted, and no `--` separator is emitted before the task.
...
Then inside `buildClaudeCmdParts`, immediately after the `if (effort) { parts.push("--effort", effort); }` block, add the `--allowedTools` emit:
...
**Then replace the existing final line** — `parts.push(shellEscape(input.task));` at `pi-extension/subagents/index.ts:687` — with a separator-aware emit. This is the v5 / review-v6 blocker 1 fix:

```ts
  // Emit `--` before the task so the variadic --allowedTools option
...
Do not emit a bare `parts.push(shellEscape(input.task))` outside the `if`. The upstream contract is "no `--` when no task", and the fork builder must match so the commit is drop-in portable to an upstream PR. Callers that genuinely need to launch Claude without a task (not a use case we have today) would omit both the separator and the task; the builder just returns the arg array without either.
...
git commit -m "feat(subagents): emit --allowedTools + required -- separator on Claude path

Adds PI_TO_CLAUDE_TOOLS map + --allowedTools emission, closing the
tool-restriction security regression identified during fork-state
review. Also emits the -- task separator required by Claude CLI's
variadic --allowedTools option, mirroring the upstream regression
test at pi-subagent/test/claude-args.test.ts:302-316 so
tool-restricted runs do not have their prompt consumed as another
tool name. Kept as a discrete named commit portable to an upstream
...
This Task closes review findings 2 (parity), 4 (`resumeSessionId`), and the review-v6 blocker 2 (transcript archival must not depend on a guessed Claude project-dir slug). The pi headless path already consumes `ResolvedLaunchSpec`; the Claude headless path now does too. Specifically the Claude args include:
...
export function buildClaudeHeadlessArgs(
  ...
  // Append `--` before the task so the variadic --allowedTools option does
  // not swallow the prompt as "another tool name" — mirrors the pane-side
