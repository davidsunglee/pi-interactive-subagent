# Mux-Free Execution Implementation Plan (v18)
...
**Architecture:**
1. **Shared launch contract.** Extract `resolveLaunchSpec()` so pane and headless consume the same normalized launch behavior: agent defaults, cwd/config-root resolution, system-prompt handling, Claude model normalization, lineage seeding, pi artifact task delivery, deny-tools, auto-exit, skill expansion, session-file placement, and `resumeSessionId`. Artifact-backed task delivery is pi-specific; the Claude CLI prompt argument does not support `@file` substitution, so Claude pane and headless always pass `spec.claudeTaskBody` directly.
...
3. **Backend-aware tool entrypoints.** Route all three subagent surfaces through backend-aware preflight and dispatch. The bare `subagent` headless path registers background work in the shared `runningSubagents` map (widened to allow headless entries) so the existing `session_shutdown` / `/reload` cleanup aborts it — headless background work cannot become untracked fire-and-forget.
...
- **Claude task delivery:** Claude pane and headless always pass `spec.claudeTaskBody` directly as the prompt argument. `spec.taskDelivery === "artifact"` is a pi-path signal and is not honored on Claude paths because the Claude CLI prompt argument does not support `@file` substitution.
- **Claude model normalization:** `resolveLaunchSpec()` computes `spec.claudeModelArg` by stripping any provider prefix once, and both pane/headless Claude builders consume that shared field. Neither builder should slice `effectiveModel` on its own.
...
- **Headless bare-subagent lifecycle:** no-mux `subagent` launches register into the shared `runningSubagents` map at launch time, which the existing `session_shutdown` handler already sweeps; `/reload` drops the whole map with the old module. Headless background work cannot become an untracked fire-and-forget process.
- **Vestigial `interactive` field:** accepted on `SubagentParams` and `OrchestrationTaskSchema` for legacy callers, but has no runtime effect in v1. Neither backend branches on it.
...
- `pi-extension/subagents/launch-spec.ts` — shared `resolveLaunchSpec()` plus task/system-prompt artifact helpers and one-shot Claude model normalization (`claudeModelArg`).
...
- `pi-extension/subagents/backends/claude-stream.ts` — Claude arg builder (consuming shared `spec.claudeModelArg`) plus stream/result parsing.
...
- `test/orchestration/launch-spec.test.ts` — launch-contract normalization, shared Claude model normalization, and Claude identity/task-body rules.
...
- `test/orchestration/interactive-compat.test.ts` — vestigial `interactive` field is accepted and ignored.
- `test/orchestration/headless-onupdate-replay.test.ts` — buffered replay of partials emitted before `watch()` attaches its `onUpdate`.
...
- `test/integration/headless-claude-smoke.test.ts` — headless Claude smoke with explicit cache-busting for the `usage.cost > 0` gate.
...
- `test/integration/agents/test-long-running.md` — fixture that stays alive indefinitely, used by the shutdown-abort case in `orchestration-headless-no-mux`.

**Modified files**
- `pi-extension/subagents/cmux.ts` — export `detectMux()`.
- `pi-extension/subagents/index.ts` — baseline type fixes; `resolveLaunchSpec()` adoption; backend-aware preflight; bare `subagent` headless path with lifecycle tracking; Claude `--tools` patch; shared Claude model normalization via `spec.claudeModelArg`; shared Claude-skills warning; `shellEscape` re-export.
- `pi-extension/orchestration/default-deps.ts` — backend selection/dispatch plus headless result/update mapping.
- `pi-extension/orchestration/types.ts` — optional `usage` / `transcript` fields, vestigial `interactive` field on `OrchestrationTaskSchema`, and update signatures.
...
### Task 4b: Accept the vestigial `interactive` field on public schemas (compat)

**Files:**
- Modify: `pi-extension/subagents/index.ts` (extend `SubagentParams`)
- Modify: `pi-extension/orchestration/types.ts` (extend `OrchestrationTaskSchema`)
- Create: `test/orchestration/interactive-compat.test.ts`

The spec requires that legacy callers sending the `interactive` field continue to validate. The field has no behavioral effect in either backend — this task only widens the public schemas so validation does not reject it, and adds one focused test proving the field is tolerated and ignored.
...
Note: the `resolveLaunchSpec` case depends on Task 9b landing `launch-spec.ts`. Until Task 9b runs, skip that case or mark it with `{ skip: true }`; wire it in when Task 9b's Step 3 completes. The two schema cases run independently and should pass after Step 2 below.

- [ ] **Step 2: Run the test and confirm it fails**

Run: `npm test -- --test-name-pattern='interactive field is accepted' 2>&1 | tail -30`
Expected: the first two `Value.Check(...)` assertions FAIL with `=== false`, because neither `SubagentParams` nor `OrchestrationTaskSchema` currently declares `interactive`.

- [ ] **Step 3: Add `interactive` to the public schemas**

In `pi-extension/subagents/index.ts`, inside the `SubagentParams` `Type.Object({...})` block (currently around line 58–113), append:
...
In `pi-extension/orchestration/types.ts`, inside the `OrchestrationTaskSchema` `Type.Object({...})` block (currently around line 4–27), replace the trailing `interactive` / `permissionMode` comment with:
...
- [ ] **Step 4: Re-run the test; expect all schema cases pass**

Run: `npm test -- --test-name-pattern='interactive field is accepted' 2>&1 | tail -30`
Expected: both schema cases PASS. The `resolveLaunchSpec` case is still `skip` (or failing with "module not found") until Task 9b lands; that is expected.

- [ ] **Step 5: Typecheck**

Run: `npm run typecheck 2>&1 | tail -20`
Expected: no errors. `OrchestrationTask` now carries an optional `interactive?: boolean`, which is structurally assignable to `BackendLaunchParams` (which accepts the same ignored field per Task 4).

- [ ] **Step 6: Follow-up wire-in inside Task 9b**

When Task 9b implements `resolveLaunchSpec()`, do **not** read `params.interactive` — simply leave the field unread. Once Task 9b's Step 3 lands, unskip the `resolveLaunchSpec()` case in this test file so it proves the resolved spec carries no `interactive`-influenced difference.

- [ ] **Step 7: Commit**

```bash
git add pi-extension/subagents/index.ts pi-extension/orchestration/types.ts test/orchestration/interactive-compat.test.ts
git commit -m "feat(schemas): accept vestigial \`interactive\` field on SubagentParams + OrchestrationTaskSchema

Legacy callers that still send \`interactive\` now validate cleanly
instead of being rejected at the schema boundary. The field remains
ignored at runtime in both the pane and headless backends; see the
spec's out-of-scope note for why it is intentionally inert in v1."
```
...
**Files:**
- Create: `pi-extension/subagents/backends/headless.ts` (Phase 1 stub; Phase 2 / Task 11 overwrites it)
...
After this Step the preflight gate is backend-aware for all three subagent tools. The orchestration tools already dispatch launch/watch through `default-deps.ts` → `selectBackend()` (Task 8), so the preflight swap alone makes them functional in headless. The bare `subagent` tool still calls the pane-only `launchSubagent` / `watchSubagent` directly, so after Step 4 a headless invocation would pass preflight and then fail at launch — **Step 4a lands the headless stub that Step 4c imports, Step 4b widens the `RunningSubagent` shape so headless entries fit in the existing shutdown-tracked map, and Step 4c rewires the bare `subagent` execute to dispatch through the headless backend while registering the run for shutdown/reload cleanup.**

- [ ] **Step 4a: Create the Phase 1 headless stub so Step 4c and Task 8 can import it**

Create `pi-extension/subagents/backends/headless.ts` (Phase 2 / Task 11 overwrites this file with the real implementation):
...
This must land before Step 4c because Step 4c imports `makeHeadlessBackend` from this file. The stub is also the module Task 8 Step 3 rewires `default-deps.ts` around.

- [ ] **Step 4b: Widen `RunningSubagent` to accept headless entries so the existing shutdown/reload cleanup covers them**

This is the first half of the lifecycle wire-in. The existing `session_shutdown` handler in `pi-extension/subagents/index.ts` (around line 1132–1144) already iterates every entry in the module-level `runningSubagents` map and aborts its `abortController`, and the top-of-file `/reload` block (around line 37–52) already re-runs on every module re-import. If a bare headless launch is registered in the same map as the pane launches, both cleanup paths find and abort it without any further wiring. The only reason that does not currently hold is that `RunningSubagent` requires pane-specific fields (`surface`, `sessionFile`) that the headless path has no equivalent of.

Edit `RunningSubagent` (currently around line 458–472) so the pane-specific fields become optional and a `backend` discriminator is added:
...
Every existing pane call site that constructs a `RunningSubagent` (currently around lines 779, 950, and 1523) must now also set `backend: "pane"`. Typecheck will catch any that was missed:

```
npm run typecheck
```

The widget renderer at line 596 only reads `name`, `agent`, `startTime`, `entries`, `bytes` — all still defined — so no widget code changes are required. The shutdown handler is already correct as written (it only reads `abortController`); leave it unchanged.

- [ ] **Step 4c: Rewire the bare `subagent` tool's execute to branch on `selectBackend()`, registering the headless run in `runningSubagents`**

Still in `pi-extension/subagents/index.ts`, inside the bare `subagent` tool's `execute` body (around line 1175–1256), branch on `selectBackend()` right after the `preflightOrchestration` check. The pane branch is the existing flow unchanged (widget refresh, ping-via-sentinel handling, steer messages). The headless branch dispatches through the headless backend adapter, **registers the run in the shared `runningSubagents` map so `session_shutdown` aborts it**, and delivers results through the same steer-message pattern but without pane-only glue.
...
Lifecycle invariants this Step establishes and that must hold before moving on:

1. **Tracked at launch:** the `runningSubagents.set(id, running)` call happens synchronously before the watch promise is awaited, so even a same-tick `session_shutdown` aborts the run.
2. **`session_shutdown` aborts headless runs:** the existing shutdown handler (line 1140–1143) iterates `runningSubagents` and calls `agent.abortController?.abort()` on every entry. Because the headless branch stores `watcherAbort` as `abortController`, shutdown reaches it with no further wiring.
3. **`/reload` clears stale entries:** `/reload` re-imports the module, which both (a) aborts the previous `POLL_ABORT_KEY` controller (lines 49–51, pi-side cleanup) and (b) produces a fresh `runningSubagents` map in the new module's closure. The headless watch promise from the previous module will observe `watcherAbort.signal.aborted === false` (its AbortController is still live in the old closure), but its `.finally()` runs inside the *old* module's closure and deletes from the *old* map, which is already unreachable — so no entry leaks into the new module's state.
4. **No double-completion steer:** the `.finally()` block only deletes the map entry; the result/error steer is emitted exactly once from the `.then` or `.catch` branch.
...
Task 23b's `session_shutdown`-abort case is the execution-level regression guard for this wire-in. Do not land Task 7b without confirming Task 23b's shutdown assertion is green once Phase 2 lands the real headless backend.
...
The existing pane integration tests (Phase 0) and the no-mux integration tests (Task 23b) are the execution-level regression guards for Steps 4b–4c. After this Task they both cover the bare `subagent` tool path — confirm in Task 23b that the test file includes a `subagent` case alongside `subagent_serial` / `subagent_parallel` **and** that it exercises the `session_shutdown`-abort assertion against a tracked headless run.

- [ ] **Step 6: Commit**

```bash
git add pi-extension/subagents/backends/headless.ts pi-extension/subagents/preflight-orchestration.ts test/orchestration/preflight-orchestration.test.ts pi-extension/subagents/index.ts
git commit -m "feat(orchestration): backend-aware preflight + bare-subagent headless dispatch with shutdown-tracked lifecycle

Register headless bare-subagent runs in the shared runningSubagents
map so the existing session_shutdown handler aborts them; widens the
RunningSubagent shape to make pane-only fields optional and adds a
backend discriminator. Unblocks no-mux subagent/subagent_serial/
subagent_parallel without orphaning background work across shutdown
or /reload."
```
...
- [ ] **Step 3: Rewire `default-deps.ts` to dispatch through `selectBackend()`**

The Phase 1 headless stub already exists at `pi-extension/subagents/backends/headless.ts` (landed by Task 7b Step 4a). Rewrite `pi-extension/orchestration/default-deps.ts`:
...
export interface ResolvedLaunchSpec {
  ...
  claudeModelArg: string | undefined;      // shared Claude-only projection of effectiveModel
...
- `claudeModelArg` (new in v18, review-v21 blocker) — a Claude-only projection derived once from `effectiveModel`: strip a leading `<provider>/` prefix when present (`anthropic/claude-haiku-4-5` → `claude-haiku-4-5`), otherwise keep the model unchanged. Keep `effectiveModel` raw for the pi path and for reporting; both Claude builders must consume `claudeModelArg` instead of re-normalizing.
...
Move the artifact-write side-effects (system-prompt file at line 855-858, task artifact at line 905-916) from `launchSubagent` into helper functions inside `launch-spec.ts` (`writeSystemPromptArtifact(spec)`, `writeTaskArtifact(spec)`) so the pane and headless paths share them. `writeTaskArtifact(spec)` writes `spec.fullTask` for the pi path. Artifact delivery is pi-only in v1: pane pi and headless pi both call `writeTaskArtifact(spec)` when `spec.taskDelivery === "artifact"`; Claude pane and Claude headless always pass `spec.claudeTaskBody` directly because the Claude CLI prompt argument does not support `@file` substitution. There is no `flavor` option on `writeTaskArtifact` in v1.
...
Expected: at least one hit — the Phase 1 stub string (set by Task 7b Step 4a). Phase 2 / Task 11 overwrites this file and removes the stub; its presence here confirms we have not accidentally landed the real backend ahead of schedule.
...
Expected: at least one hit inside the bare `subagent` tool's `execute` (from Task 7b Step 4c).

Finally, confirm the headless branch registers into `runningSubagents` so shutdown/reload cleanup reaches it:

Run: `grep -n 'backend: "headless"' pi-extension/subagents/index.ts`
Expected: at least one hit inside the headless branch of the bare `subagent` `execute`, tagging the `RunningSubagent` entry that gets `.set(id, running)`. If absent, Task 7b Step 4c did not wire the lifecycle registration — fix before moving on.
...
- **Defer the registration swap.** In `pi-extension/subagents/index.ts`, keep `preflightSubagent` at both the orchestration-tool registration call and inside the bare `subagent` tool's `execute`, and add a short inline note pointing at Task 11. Swap to `preflightOrchestration` at both sites as one commit immediately after Task 11 lands. If you pick this path, also defer Task 7b Steps 4b–4c (the `RunningSubagent` widening and bare-subagent execute `selectBackend()` branch) so the three changes land together — otherwise preflight still blocks on mux while the execute tries to dispatch headless, or vice versa, or the headless execute compiles against an untracked `RunningSubagent` shape. Stage all three changes as a single commit and hold it until Phase 2 is ready.
...
A regression of review-v11 finding 1 (someone "simplifying" this back to `params.systemPrompt ?? agentDefs?.body` and `params.task`) is caught by the new mixed-case test in `launch-spec.test.ts` (Step 1) plus a new pane-side unit test added to Step 1 of this task. **v18 (review-v21 blocker):** the pane call site must also consume `spec.claudeModelArg`, not raw `effectiveModel`, so provider-prefixed Claude models behave identically on pane and headless:
...
If `test/orchestration/thinking-effort.test.ts` does not already import `resolveLaunchSpec`, add `import { resolveLaunchSpec } from "../../pi-extension/subagents/launch-spec.ts";` next to the existing `buildClaudeCmdParts` / `shellEscape` imports so the new pane-regression case can build its argv from the shared normalized spec.
...
git commit -m "feat(subagents): emit --tools + required -- separator on Claude path; share model normalization + skills warning; centralize tool map; unify Claude system-prompt contract
...
identity source, placement flag, task body, and normalized
Claude model arg. Kept as a discrete named commit portable to an
...
The abort path should continue to use the same explicit `exited`-flag pattern as `runPiHeadless`. **v18 (review-v21 blocker):** do not strip provider prefixes inside `buildClaudeHeadlessArgs`; headless must consume the same pre-normalized `spec.claudeModelArg` field the pane path uses.
  ...
  return new Promise<BackendResult>((resolve) => {
    ...
    const processLine = (line: string) => {
      ...
      if (event.type === "result") {
        // The terminal `result` event finalizes `usage`, `finalMessage`, and error
        // state. The final assistant turn has already been appended to
        // `transcript[]` via the preceding streamed `assistant` event — do NOT
        // append it again here, or `transcript[]` will record the last turn twice.
...
And add `readdirSync` to the `node:fs` import list at the top of `headless.ts` (alongside the existing `copyFileSync, mkdirSync`).
...
git commit -m "feat(backends): headless Claude consumes ResolvedLaunchSpec; share Claude model normalization; thread --resume; exit-tracked SIGKILL; session-id transcript discovery; lock shared tool-map coverage; iterate tool_result messages into transcript[] (review-v15 / review-v21 findings)"
```
...
- [ ] **Step 1: Write the test with an explicit cache-busting marker**
...
describe("headless-claude-smoke", { skip: !CLAUDE_AVAILABLE, timeout: 120_000 }, () => {
  ...
  it("runs a unique-marker Claude task with non-zero cost, sessionId, archived transcript", async () => {
    ...
    // A regression that re-adds `transcript.push({ role: "assistant", ... finalOutput })`
    // in the `result` branch of runClaudeHeadless would make the final message appear
    // twice back-to-back.
...
git commit -m "test(integration): add cache-busted headless Claude smoke test"
```
...
Append to `test/integration/headless-transcript-archival.test.ts` (after the existing `describe(... [pi] ...)` block). The assertions below mirror the pi half: the archived file must exist, be non-empty, contain the session id, and contain the task prompt text, so a wrong-file copy or stale same-basename file is not silently accepted.
...
Note: `readFileSync` and `homedir` are already imported at the top of the file from the pi-half test — keep a single import of each.
...
**Files:**
- Create: `test/integration/agents/test-long-running.md`
...
The shutdown-abort case needs a fixture agent that actually stays alive long enough for `session_shutdown` to fire while it is still running. `test-ping` calls `caller_ping` once and exits, so it cannot prove the abort path — the run may complete before shutdown even arrives. Create a dedicated long-running fixture for this case.
...
`copyTestAgents()` (from Task 12b) reads every `.md` file under `test/integration/agents/`, so adding this fixture makes it available to both `PI_SUBAGENT_MODE=headless` and auto-mode integration runs without any harness change.
...
Expected: all seven tests PASS (forced-headless: bare `subagent` success, bare `subagent` shutdown-abort, `subagent_serial`, `subagent_parallel`; auto + no-mux: `subagent`, `subagent_serial`, `subagent_parallel`), or all seven skip when `pi` is missing. A no-mux regression on any of the three tool surfaces manifests as its case failing with either a "preflight blocked headless dispatch" message or a "bare subagent preflight blocked" message. The shutdown case specifically guards the lifecycle invariant from Task 7b: once the headless bare-`subagent` run is registered against a fixture that does not exit on its own (`test-long-running`), `session_shutdown` must abort it and must not allow a later orphaned success completion. The auto-mode cases (Case B) are the path the feature sells as the user default, so Case B must not be skipped or weakened when Case A passes.

- [ ] **Step 4: Commit**

```bash
git add test/integration/agents/test-long-running.md test/integration/orchestration-headless-no-mux.test.ts
git commit -m "test(integration): exercise subagent/subagent_serial/subagent_parallel via real registered tool callbacks under forced-headless and auto+no-mux; assert bare-subagent shutdown abort with a fixture that stays alive"
```
...
Also expose a test-only hook at the bottom of `headless.ts` so the replay invariant can be exercised deterministically without spawning a CLI:

```ts
export const __test__ = {
  /**
   * Same as `makeHeadlessBackend` but routes both pi and Claude paths
   * through `runner`. Tests use this to drive deterministic emit/resolve
   * sequences; the backend's buffering/replay machinery is unchanged.
   */
  makeHeadlessBackendWithRunner(
    ...
    // identical to makeHeadlessBackend(ctx), except the runPiHeadless /
    // runClaudeHeadless branch is replaced with:
    //   runner({ spec, startTime, abort: abort.signal,
    //            emitPartial: (snap) => emitPartial(entry, snap) })
    // where `entry` is the HeadlessLaunch created for this launch.
  },
};
```
...
- [ ] **Step 5: Add a focused regression test for the pre-attach partial replay invariant**

Create `test/orchestration/headless-onupdate-replay.test.ts`:
...
Run: `npm test -- --test-name-pattern='headless backend onUpdate' 2>&1 | tail -40`
Expected: both cases PASS. If the first case fails, Step 2's buffering/replay wiring is missing (the `latestPartial` snapshot on the `HeadlessLaunch` entry is not being replayed when `watch()` attaches). If the second case fails, post-attach partials are not flowing through — likely `entry.onUpdate` is not being invoked from `emitPartial`.
...
```bash
git add pi-extension/subagents/backends/headless.ts pi-extension/subagents/backends/types.ts pi-extension/orchestration/default-deps.ts pi-extension/orchestration/types.ts pi-extension/orchestration/tool-handlers.ts pi-extension/orchestration/run-serial.ts pi-extension/orchestration/run-parallel.ts test/orchestration/headless-onupdate-replay.test.ts
...
- Plumbing the `interactive` schema field as a real behavioral switch. The field is accepted on `SubagentParams` and `OrchestrationTaskSchema` (Task 4b) but remains inert in v1 — neither backend branches on it.
