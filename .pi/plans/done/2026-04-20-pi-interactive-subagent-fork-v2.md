# pi-interactive-subagent Fork Implementation Plan (v2)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

> **v2 revision notes** (addresses `.pi/plans/reviews/2026-04-20-pi-interactive-subagent-fork-review.md`):
> 1. Tool registration gating now fires per-tool — Task 17 accepts a `shouldRegister` predicate, Task 18 passes it. No more bulk-register when only one tool is allowed.
> 2. Completion path is unambiguous: default-deps uses `watchSubagent()` as the completion primitive, then calls `readTranscript(running)` to derive a uniform `finalMessage` + `transcriptPath` across both pi and Claude branches. The standalone `sentinel-wait.ts` primitive was dropped — it would have been dead code, since `watchSubagent` already polls via `pollForExit`. (Tasks 15–16 from v1 are removed; downstream task numbers shift by 2.)
> 3. `OrchestrationTaskSchema` drops unused `interactive` / `permissionMode` fields (they were never plumbed to `launchSubagent`).
> 4. `caller_ping` surfacing is now explicitly deferred — the misleading v1 Task 17 claim that ping was inherited has been removed. Aligned with the Deferred section.
> 5. `npm test` script includes the existing `test/system-prompt-mode.test.ts` plus every new orchestration test file. Final file-layout check in Task 21 matches exactly.
> 6. Tool-signal → running-wait cancellation propagation is explicitly called out as deferred (Task 15 and Deferred section).
...
- `pi-extension/orchestration/default-deps.ts` — `makeDefaultDeps(ctx)` composing `launchSubagent` + `watchSubagent` + `readTranscript` into `LauncherDeps`.
- `pi-extension/orchestration/tool-handlers.ts` — `registerOrchestrationTools(pi, depsFactory, shouldRegister)` — conditionally registers each tool.
...
## Task 15: Build the default `LauncherDeps` bound to real `launchSubagent`

**Files:**
- Create: `pi-extension/orchestration/default-deps.ts`

- [ ] **Step 1: Write the glue that composes `launchSubagent` + `watchSubagent` + `readTranscript` into `LauncherDeps`**
 ...
 *
 * Completion path:
 *   launch = launchSubagent (widget registration, surface creation)
 *   waitForCompletion = watchSubagent (polling, widget updates) then
 *                       readTranscript(running) to derive a uniform
 *                       finalMessage + transcriptPath across pi/Claude.
 *
 * We reuse watchSubagent because it already owns widget lifecycle, pane
 * cleanup, and polling. We layer readTranscript on top because the
 * upstream watchSubagent result shape does not carry a uniform transcript
 * path (in particular, Claude's .transcript pointer is only surfaced via
 * the RunningSubagent side-state, not the returned summary object).
 *
...
export function makeDefaultDeps(ctx: {
  ...
  return {
    ...
    async waitForCompletion(handle: LaunchedHandle): Promise<OrchestrationResult> {
      ...
      const transcript = await readTranscript(running);
      handleToRunning.delete(handle.id);
      return {
        name: handle.name,
        finalMessage: transcript.finalMessage || sub.summary,
        transcriptPath: transcript.transcriptPath,
        exitCode: sub.exitCode,
        elapsedMs: sub.elapsed * 1000,
        sessionId: transcript.claudeSessionId ?? sub.claudeSessionId,
        error: sub.error,
      };
    },
  };
...
Run: `npm test`
Expected: all green (default-deps.ts is not imported yet; compiles via tests touching it only after Task 17).
...
## Task 16: Write failing test for `registerOrchestrationTools` tool surface
...
## Task 17: Implement `tool-handlers.ts`
...
## Task 18: Wire orchestration tools into the extension default export
...
## Task 19: Write the Claude sentinel roundtrip integration test (local-only)
...
## Task 20: README updates — tool docs + Claude plugin install + smoke checklist
...
## Task 21: Final sweep — run everything, confirm green
...
default-deps.ts   run-parallel.ts   run-serial.ts   tool-handlers.ts   transcript-read.ts   types.ts

test/orchestration:
run-parallel.test.ts   run-serial.test.ts   thinking-effort.test.ts   tool-handlers.test.ts   transcript-read.test.ts
```
