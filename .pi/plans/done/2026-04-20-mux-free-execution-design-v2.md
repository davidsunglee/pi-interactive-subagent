# Mux-Free Execution Implementation Plan (v2)
...
**Architecture (v2 — three layered changes):**
...
**New files (shared launch resolution + headless backend + tests):**
...
- `package.json` — add `@mariozechner/pi-ai` to both `peerDependencies` and `devDependencies` so the headless backend's `Message` import is no longer dependent on transitive resolution from upstream (review note 3, Task 4 Step 2).
...
import assert from "node:assert/strict";
import { preflightOrchestration } from "../../pi-extension/subagents/preflight-orchestration.ts";
...
import { muxSetupHint } from "./cmux.ts";
...
export function preflightOrchestration(ctx: {
  ...
  if (selectBackend() === "pane") {
    ...
    // Inline isMuxAvailable() check to avoid double-import; selectBackend
    // already called detectMux() to reach this branch only if mux was true,
    // so getting here means pane mode + (mux present OR mode=pane override).
    // We re-check explicitly so a forced PI_SUBAGENT_MODE=pane without mux
    // still surfaces the helpful setup hint.
    // (Importing isMuxAvailable here directly avoids the alias indirection.)
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const { isMuxAvailable } = require("./cmux.ts") as typeof import("./cmux.ts");
    if (!isMuxAvailable()) {
      return {
        content: [
...
Note on the dynamic `require()`: the project is ESM (`"type": "module"`). If `require` is unavailable in the runtime, replace the call with a top-level `import { isMuxAvailable } from "./cmux.ts";` instead — the only reason to defer was to keep the import surface tight. Confirm by running the unit test; if it works with the static import, prefer that.
...
/**
 * Minimal ExtensionAPI shim — captures registered tools and forwards
 * session_start so the extension can wire its widget context. Mirrors the
 * surface area subagentsExtension actually touches.
 */
function makeFakePi() {
