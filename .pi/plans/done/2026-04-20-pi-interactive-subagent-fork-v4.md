# pi-interactive-subagent Fork Implementation Plan (v4)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

> **v4 revision notes** (addresses `.pi/plans/reviews/2026-04-20-pi-interactive-subagent-fork-review-v3.md`):
...
3. **Parameter / agent-frontmatter reference table.** If the README has a table or bulleted reference of `SubagentParams` fields (or the agent-frontmatter fields), add rows / bullets for the three new fields — matching the schema descriptions from Task 4 Step 1:
...
If the plugin is not installed and a `cli: "claude"` task is dispatched, the Claude process completes but no Stop-hook sentinel is ever written, so `watchSubagent` keeps polling until the caller cancels the subagent (through the widget or by aborting the tool call). This fork does **not** currently emit a dedicated "plugin not installed" error or apply a fixed timeout — diagnosing a missing install is a manual step. Auto-install and an installation-health probe are listed under Deferred work.
