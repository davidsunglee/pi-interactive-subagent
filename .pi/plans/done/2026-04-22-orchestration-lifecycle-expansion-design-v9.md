# Orchestration Lifecycle Expansion Implementation Plan (v9)
...
- Step 12.2a — `resume-tool-boundary.test.ts` uses `setWatchSubagentOverride(...)` to control resume outcomes, `getRegistry` / `resetRegistry` to seed ownership state and isolate cases, **and `setMuxAvailableOverride(true)` so the boundary cases reach the post-mux-gate registry routing on no-mux hosts**.
...
Write a focused unit-level test against the registered `subagent_resume` tool's `execute` handler. The test uses three `__test__` seams from Task 7b:
...
describe("subagent_resume tool boundary", () => {
  ...
  it("sessionPath path routes terminal resume through registry.onResumeTerminal", async () => {
    const ownedPath = "/tmp/owned.jsonl";
    const id = registry.dispatchAsync({
      config: { mode: "serial", tasks: [{ name: "a", agent: "x", task: "t" }] },
    });
  ...
  it("unowned sessionPath resumes cleanly (standalone non-orch path) without mutating registry", async () => {
    ...
    const out = await getResume().execute("c6",
      { sessionPath: "/tmp/stray.jsonl", message: "go" },
      new AbortController().signal, () => {}, toolCtx());
