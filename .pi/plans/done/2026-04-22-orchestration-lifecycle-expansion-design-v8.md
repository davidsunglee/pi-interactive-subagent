# Orchestration Lifecycle Expansion Implementation Plan (v8)
...
- Claude children's `sessionKey` is late-bound via a concrete backend seam: `Backend.watch` carries an `onSessionKey` callback (Step 9.5b Part A). Headless Claude fires it from the `system/init` event parser (Part C). Pane Claude fires it from two sites (Part B) — a best-effort early read inside `watchSubagent`'s `pollForExit.onTick`, and a **load-bearing synchronous read immediately before `watchSubagent` resolves its promise** (the race-closer against a `caller_ping` that fires between polling ticks). That callback reaches `registry.updateSessionKey` synchronously via `LauncherDeps.waitForCompletion(hooks.onSessionKey)` → runner `onSessionKey` → tool-handler dispatcher (Parts D–F), so by the time the same `await deps.waitForCompletion(...)` resolution routes a `ping` to `registry.onTaskBlocked`, the ownership map already contains the key. Pi children bind at launch via `LaunchedHandle.sessionKey` / `onTaskLaunched`.
...
- Step 12.2a — `resume-tool-boundary.test.ts` uses `setWatchSubagentOverride(...)` to control resume outcomes and uses `getRegistry` / `resetRegistry` to seed ownership state and isolate cases.
...
**Why this is race-free.** The runner site is:
...
`deps.waitForCompletion` is `backend.watch(handle, signal, hooks?.onSessionKey)` (Part D). `backend.watch` is `watchSubagent` through the pane adapter. `watchSubagent` invokes `opts.onSessionKey` **synchronously** via `maybeFire` on its final pre-return line, before resolving the outer promise. That fire chains synchronously through `opts.onSessionKey → hooks.onSessionKey → deps.waitForCompletion's hook param → runner's onSessionKey → tool-handler onSessionKey → registry.updateSessionKey`. Only after all of that does the `await` resolve, and only then does the runner read `result.ping` and call `opts.onBlocked → registry.onTaskBlocked`. So `registry.updateSessionKey` is guaranteed to run before `registry.onTaskBlocked` reads the ownership map, and the blocked steer-back carries the canonical `sessionKey`.
...
Write a focused unit-level test against the registered `subagent_resume` tool's `execute` handler. Use `__test__.setWatchSubagentOverride(...)` (Task 7b) so the test controls the resumed-session outcome without spawning a real child, and use the fake-`pi` harness pattern from `test/integration/orchestration-extension-async.test.ts` so the test exercises the real `subagentsExtension(api)` registration site. Do NOT require `pi` or a mux backend — the test is about the tool parameter contract and the registry dispatch, not the backend launch.
...
describe("subagent_resume tool boundary", () => {
  ...
  beforeEach(() => {
    ...
    registry = __test__.getRegistry(); // expose via __test__ (Task 7b-extension)
  });
...
- `subagent_resume` accepts `sessionPath` OR `sessionId` → Step 12.2 (wiring) + Step 12.2a (tool-boundary tests covering XOR validation + both-path routing)
