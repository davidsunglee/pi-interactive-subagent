...
> **v7 revision notes** (addresses `.pi/plans/reviews/2026-04-20-pi-interactive-subagent-fork-review-v6.md`):
...
> 2. **Task 18 Step 3's missing-plugin wording no longer references a non-existent widget cancel action.** v6 still told users to cancel a hung Claude wait "through the widget" and described "the widget's per-subagent cancel action" — neither exists in the codebase or in this plan. The only code path that aborts `RunningSubagent.abortController` today is `session_shutdown`. The sentence is rewritten to describe the actual recovery paths (end the pi session / restart, kill the Claude pane directly via the mux) and to cross-reference the Deferred-work entries that would add a real cancel path.
