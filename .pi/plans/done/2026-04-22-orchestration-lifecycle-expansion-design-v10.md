# Orchestration Lifecycle Expansion Implementation Plan (v10)
...
4. **Distinct `caller_ping` surfacing at the backend boundary, plus mid-run `sessionKey` propagation.** Both backends already detect `.exit` ping sidecars (pane path) or can be extended to detect them (headless); the backends propagate `ping?: { name, message }` plus `sessionKey` on their `BackendResult` so the registry can transition `running → blocked`, key ownership to the same value `subagent_resume` will receive, and emit a `blocked` steer-back carrying that key in the `sessionKey` field. The `Backend.watch(...)` seam itself gains a new optional `hooks` parameter that carries `onSessionKey` (Step 9.5b). The new `hooks` parameter is **purely additive** — `Backend.watch`'s existing `signal` and `onUpdate` positional parameters and the existing `LauncherDeps.waitForCompletion(handle, signal?, onUpdate?)` signature are preserved unchanged so today's abort propagation and partial-update behavior keep working. For headless Claude the callback fires from the `system/init` event parser. For pane Claude the callback fires from two sites: (a) a best-effort early read inside `watchSubagent`'s `pollForExit.onTick` (fast path for long-running children) and (b) a **load-bearing synchronous read immediately before `watchSubagent` returns any result carrying `ping`** — this closes the race where a child emits `caller_ping` between two polling ticks, ensuring `onSessionKey` has already fired by the time the ping result is handed back to the runner. That callback threads through `LauncherDeps.waitForCompletion`'s new `hooks` arg → runner → tool-handler → `registry.updateSessionKey` synchronously, so the ownership map is populated before the same promise resolution routes the ping to `registry.onTaskBlocked`.
...
- **Ownership key (`sessionKey`):** one canonical identifier is threaded end-to-end. For pi-backed children, `sessionKey = subagentSessionFile` (a file path — identical to what `subagent_resume({ sessionPath })` accepts). For Claude-backed children, `sessionKey = claudeSessionId`. The registry's ownership map, the blocked notification's `sessionKey` payload field, and the `subagent_resume` lookup all use this same value. Claude children's `sessionKey` is not known at launch time; it is late-bound via an `onSessionKey` callback added to the `Backend.watch` seam through a new optional `hooks` parameter (Task 9.5b). The new `hooks` parameter is **purely additive**: `Backend.watch`'s existing positional `signal` and `onUpdate` parameters and `LauncherDeps.waitForCompletion`'s existing `(handle, signal?, onUpdate?)` shape are preserved unchanged so existing abort propagation and partial-update behavior continue to work. Headless Claude fires the callback from the `system/init` event parser. Pane Claude uses two fire sites that together are race-free against `caller_ping`: a best-effort early read inside `watchSubagent`'s `pollForExit.onTick` (so long-running children populate ownership within one polling interval) and a **load-bearing synchronous read immediately before `watchSubagent` returns** — whenever the backend is about to resolve with a `ping`-carrying result (or any result at all), it first re-reads the `.transcript` pointer file and fires `onSessionKey` synchronously if the id is now known and hasn't been fired yet. That callback flows `backend → LauncherDeps.waitForCompletion(..., hooks.onSessionKey) → run-serial/run-parallel.onSessionKey → tool-handlers.onSessionKey → registry.updateSessionKey(orchestrationId, taskIndex, sessionKey)` synchronously, so by the time the same promise's resolution routes the ping to `registry.onTaskBlocked`, the ownership map is already populated. `subagent_resume` accepts either `sessionPath` (pi-backed) or `sessionId` (Claude-backed); both funnel through the same ownership-map lookup so blocked routing and cross-orch re-ingestion work for both backends (Task 12).
...
- **No per-task intermediate completion pings.** Phase 2's `blocked` is the single pre-terminal user-facing notification kind. The registry does expose two internal in-process subscriber hooks (neither is a notification): `onTaskTerminal` (fires on per-task terminal transitions — the extension uses it to drop virtual blocked widget rows) and `onResumeStarted` (fires on the `blocked -> running` leg when `subagent_resume` begins on an owned sessionKey — the extension uses it to drop the virtual blocked widget row at resume-start time, not only at terminal time).
- **`blocked -> running` transition on resume is load-bearing.** When `subagent_resume.execute` passes the XOR / mux / existsSync gates and has launched the resumed pane, it calls `registry.onResumeStarted(sessionKey)` **before** awaiting the watcher. This transitions the owning slot's snapshot state from `blocked` back to `running` (spec's lifecycle contract) and synchronously fires the `RegistryHooks.onResumeStarted` widget hook so the virtual blocked widget row is removed on the same tick the real resumed-pane `RunningSubagent` row appears — preventing a double row for the same task. Ownership is preserved across the call so the eventual terminal result or re-ping still routes correctly.
...
- **Widget per-pane rendering preserved.** Phase 2 adds a post-pane `blocked` visual state keyed on `(orchestrationId, taskIndex)` so the row persists after the pane closes; no grouping or collapsible headers. Blocked rows clear on (a) every per-task terminal transition (completed / failed / cancelled), and (b) the `blocked -> running` transition when `subagent_resume` starts on the owning session. This second path matters because `subagent_resume` itself registers a real `RunningSubagent` for the resumed pane — without the resume-start cleanup, the virtual blocked row and the real resumed row would show side-by-side until the resumed task completed.
...
- `test/orchestration/registry.test.ts` — unit tests for registry state transitions, id generation, cancellation idempotency, concurrent runs, ownership-index populate/clear/lookup, cross-orch re-ingestion via `onResumeTerminal`, the `RegistryHooks.onTaskTerminal` per-task hook, the spec's `blocked -> running` transition via `onResumeStarted` (owned-blocked slot flips to `running` + widget hook fires; unowned sessionKey is a no-op; non-blocked slot is a no-op).
...
- `test/orchestration/resume-tool-boundary.test.ts` (Phase 2, Step 12.2a) — tool-parameter-boundary tests for the registered `subagent_resume` handler: XOR of `sessionPath` / `sessionId`, both paths routing through the same `registry.lookupOwner`, and ping-during-resume recursion through `registry.onTaskBlocked`.
...
- `test/orchestration/backend-seam.test.ts` (Phase 2, Task 9.5b) — contract test proving that a fake backend firing `Backend.watch`'s `onSessionKey` mid-run populates `registry.updateSessionKey` via the full runner → tool-handler → registry wiring, before any blocked-state routing. Also includes Part H regression cases that pin `signal` (abort) and `onUpdate` (partial-update) preservation across the post-9.5b `(handle, signal?, onUpdate?, hooks?)` signature.
- `test/orchestration/claude-resume-branch.test.ts` (Phase 2, Step 14.2d) — non-seamed coverage of the Claude `sessionId` resume branch in Step 12.2: asserts the constructed `RunningSubagent` has `cli: "claude"` + a sentinel file, the launch argv carries `--resume <sessionId>`, and registry ownership routes through the input `sessionId` (not any later-rotated id).
...
- `pi-extension/orchestration/types.ts` — add `OrchestratedTaskResult` type with `state` field; extend the existing `OrchestrationResult` with optional `state`, `index`, `sessionKey`, `ping` (Task 1.3); extend `LauncherDeps.waitForCompletion` with an additive 4th-positional `hooks?: { onSessionKey?: (key: string) => void }` parameter while preserving the existing `signal?` and `onUpdate?` parameters unchanged (Task 9.5b Part D); add `AsyncDispatchEnvelope` type; add `OrchestrationState`.
...
- `pi-extension/subagents/backends/types.ts` — add `ping?: { name: string; message: string }` and `sessionKey?: string` to `BackendResult`; extend `Backend.watch` with an additive 4th positional `hooks?: { onSessionKey?: (key: string) => void }` parameter while preserving the existing positional `signal?` and `onUpdate?` parameters unchanged (Task 9.5b Part A).
...
- `pi-extension/subagents/index.ts` — register orchestration tools with `pi` handle wired for steer-back emission; extend `watchSubagent(running, signal, opts?)` with an `onSessionKey` opt that fires (a) on the first `pollForExit` tick where the Claude `.transcript` pointer file exists and (b) synchronously, one final time, immediately before `watchSubagent` resolves its promise — the second fire site is the load-bearing race-closer against a `caller_ping` that arrives between polling ticks (Task 9.5b Part B); extend widget to render `blocked` state keyed on `(orchestrationId, taskIndex)`; in `subagent_resume.execute`, call `registry.onResumeStarted(sessionKey)` immediately after the resumed pane is registered (the spec's `blocked -> running` leg — drops the virtual blocked widget row via `RegistryHooks.onResumeStarted`), then route both the terminal result and any ping-during-resume into the registry (via `onResumeTerminal` for terminal, `onTaskBlocked` for re-ping) so cross-orch re-ingestion and recursion work through the real resume tool.
- `pi-extension/orchestration/default-deps.ts` — forward the new `hooks` 4th-arg straight through from `LauncherDeps.waitForCompletion(handle, signal, onUpdate, hooks)` into `backend.watch(handle, signal, onUpdate, hooks)`. The existing `signal` + `onUpdate` plumbing (including the BackendResult → OrchestrationResult `onUpdate` projection) is preserved unchanged (Task 9.5b Part D).
...
- Claude children's `sessionKey` is late-bound via a concrete backend seam: `Backend.watch` carries an additive 4th-positional `hooks?: { onSessionKey }` parameter (Step 9.5b Part A) **without removing or repositioning the pre-existing `signal?` (2nd positional) or `onUpdate?` (3rd positional) parameters**. Likewise `LauncherDeps.waitForCompletion(handle, signal?, onUpdate?, hooks?)` keeps its `signal` and `onUpdate` arguments and adds `hooks` as a 4th positional (Part D). Headless Claude fires the `onSessionKey` hook from the `system/init` event parser (Part C). Pane Claude fires it from two sites (Part B) — a best-effort early read inside `watchSubagent`'s `pollForExit.onTick`, and a **load-bearing synchronous read immediately before `watchSubagent` resolves its promise** (the race-closer against a `caller_ping` that fires between polling ticks). That callback reaches `registry.updateSessionKey` synchronously via `LauncherDeps.waitForCompletion(handle, signal, onUpdate, { onSessionKey })` → runner `onSessionKey` → tool-handler dispatcher (Parts D–F), so by the time the same `await deps.waitForCompletion(...)` resolution routes a `ping` to `registry.onTaskBlocked`, the ownership map already contains the key. Pi children bind at launch via `LaunchedHandle.sessionKey` / `onTaskLaunched`.
- The existing abort + partial-update contract on the `Backend.watch` / `LauncherDeps.waitForCompletion` seam is preserved end-to-end after Task 9.5b. Existing tests for both must continue to pass without behavioral changes: `test/orchestration/headless-onupdate-replay.test.ts` (proves late `onUpdate` subscribers still receive the latest BackendResult snapshot) and `test/orchestration/headless-abort.test.ts` (proves `signal` aborts the headless watch). The runner-level abort behavior covered by existing `run-serial` / `run-parallel` tests (signal aborts an in-flight `waitForCompletion`) is also preserved. Step 9.5b Part H adds a regression test that pins both invariants on the exact post-change signature so they cannot silently regress alongside the new `hooks` arg.
- `subagent_resume` accepts `sessionPath` XOR `sessionId`; the handler rejects calls that pass neither or both. Both paths funnel into the same `registry.lookupOwner(sessionKey)` so blocked routing and cross-orch re-ingestion work for both backends. This contract is pinned at the tool-parameter boundary by `test/orchestration/resume-tool-boundary.test.ts` (Step 12.2a), not only at the registry level (Step 12.1) — the boundary test covers XOR validation, the `sessionPath` terminal path, the `sessionId` terminal path, the `sessionId` ping-during-resume recursion path, and the unowned-path fallthrough that must not crash.
...
- Virtual blocked widget rows ALSO clear at resume-start time (the spec's `blocked -> running` leg). `subagent_resume.execute` calls `registry.onResumeStarted(sessionKey)` immediately after the resumed pane is registered and before the watcher runs; the `RegistryHooks.onResumeStarted` wire-up in `subagentsExtension` drops the `virt-${oid}:${idx}` entry on the same tick the real resumed-pane `RunningSubagent` row appears, so the two never render side-by-side.
- The registry's owned-slot state transitions `blocked -> running` on `onResumeStarted` (matching the spec's state machine). `onResumeStarted` is a no-op for unowned sessionKeys and for slots that are not currently `blocked` — it never crashes, never changes ownership, and never alters state for a non-blocked slot (cancellation-race safe).
- `subagent_resume` handles both outcomes on an orch-owned session: resume-start calls `registry.onResumeStarted` (widget + state), terminal completion calls `registry.onResumeTerminal`, and ping-during-resume calls `registry.onTaskBlocked` (recursion through the real tool path).
...
export interface RegistryHooks {
   ...
   * time rather than only at terminal time — the spec's `blocked -> running`
   * transition on resume is user-visible (real RunningSubagent row opens
   * for the resumed pane) and the stale virtual row must be cleared
   * synchronously to avoid a double row.
   */
  onResumeStarted?: (ctx: {
    orchestrationId: string;
    taskIndex: number;
...
export interface Registry {
  ...
  /**
   * Called by `subagent_resume` when the resumed child actually starts on an
   * orch-owned session — AFTER the XOR / mux / existsSync gates pass and the
   * pane is about to be spawned, BEFORE the watcher is awaited. Transitions
   * the owning slot `blocked -> running` (spec's lifecycle leg), fires the
   * `onResumeStarted` extension hook for widget cleanup, and is a no-op for
   * unowned sessionKeys (standalone resumes that don't target an orch slot).
   * Does NOT mutate ownership — the sessionKey stays mapped so the eventual
   * terminal result or ping can still route back through `onResumeTerminal`
   * / `onTaskBlocked`.
   */
  onResumeStarted(sessionKey: string): void;
  /**
   * Called when a standalone `subagent_resume` finishes on an orch-owned
   * session. If the slot was `blocked` or `running` (post-`onResumeStarted`),
   * restarts serial continuation (for serial runs) and re-evaluates
   * aggregation (for parallel runs).
   */
...
Run: `node --test test/orchestration/registry.test.ts`
Expected: PASS all tests, including the three new `onResumeStarted` cases (blocked-slot transition + hook, no-op on unowned, no-op on non-blocked slot).
...
const registry: Registry = createRegistry(
  ...
  },
  {
    ...
    // Resume-start hook. Task 13 uses this to clear the virtual blocked
    // widget row the moment `subagent_resume` starts on an owned sessionKey,
    // so the resumed pane's real RunningSubagent row is not shadowed by a
    // stale virtual blocked row. Spec alignment: the `blocked -> running`
    // transition is user-visible at the widget layer. This is also NOT a
    // notification kind — internal in-process signal only.
...
Declare two module-level stubs near the top of `subagentsExtension`:

- `onOrchestrationTaskTerminal(orchestrationId, taskIndex)` — Task 13 fills it in with virtual-widget cleanup logic on per-task terminal transitions.
- `onOrchestrationResumeStarted(orchestrationId, taskIndex)` — Task 13 fills it in with virtual-widget cleanup logic on the `blocked -> running` transition.

Task 5 leaves both as no-ops so the Task 5 tests keep passing; the hook wiring is installed here (Task 5) so Task 13 only needs to fill the bodies.
...
**Rationale.** Tasks 8.3b, 10.7b, 12, and 14.2 construct the real `subagentsExtension(pi)` and must intercept three cross-cutting collaborators without mocking the registry or forking the extension module:
...
3. The `subagent_resume` handler's `isMuxAvailable()` gate — so the Step 12.2a tool-boundary test can exercise the registry-routing branches on machines with no terminal multiplexer installed (CI runners, headless dev machines). Without this seam, `subagent_resume.execute` short-circuits with `muxUnavailableResult()` before reaching the watcher override, and the boundary cases that depend on the watcher override (cases 3–6 of Step 12.2a) never run.

Today the existing `__test__` surface only exposes widget/launch-spec helpers. Land all three seams in one place, explicitly, so later tasks can reference a known API surface instead of inventing it inline.
...
Add a third module-level mutable reference for the mux availability override:

```ts
// Test seam: Task 7b (__test__.setMuxAvailableOverride). Bypasses the
// `isMuxAvailable()` short-circuit inside `subagent_resume.execute` so
// boundary tests can exercise the registry-routing branches on hosts
// with no terminal multiplexer (CI runners). null = use real check.
let muxAvailableOverride: boolean | null = null;
```

In `subagent_resume.execute` (and any other handler that depends on the same gate — currently just `preflightSubagent` and `subagent_resume`), gate `isMuxAvailable()` through the override. For the resume handler specifically, replace:
...
Production behavior is unchanged when the override is `null` (the default). Do **not** apply this override to `preflightSubagent` — orchestration tools must continue to honor the real check, since the dispatch path actually launches children and a fake "mux available" reading would fail at the backend layer.

Then extend the `__test__` export with all three seams:
...
export const __test__ = {
  ...
  // Registry introspection for tool-boundary tests (Step 12.2a). `getRegistry`
  // returns the internal registry singleton constructed in `subagentsExtension`
  // so tests can seed ownership and inspect snapshots. `resetRegistry` re-
  // constructs a fresh registry instance so tests do not bleed state across
  // cases. Both are test-only — never referenced from production code paths.
  getRegistry(): Registry { return registry; },
  resetRegistry() { registry = createRegistry(registryEmitter); },
  // Mux-availability override: lets the Step 12.2a tool-boundary test exercise
  // the post-mux-gate registry routing on no-mux hosts. null = honor the real
  // isMuxAvailable() probe (production default).
...
For `getRegistry` / `resetRegistry` to work, the `registry` binding in `subagentsExtension` must be declared with `let` (not `const`) so `resetRegistry` can reassign it, and the emitter closure must be hoisted into a top-level `registryEmitter` so the reconstructed registry uses the same emission contract. Keep the production initialization unchanged — only the declaration form and the scope of the emitter change.
...
- Step 12.2a — `resume-tool-boundary.test.ts` uses `setWatchSubagentOverride(...)` to control resume outcomes, `getRegistry` / `resetRegistry` to seed ownership state and isolate cases, **`setMuxAvailableOverride(true)` so the boundary cases reach the post-mux-gate registry routing on no-mux hosts**, and `setSurfaceOverrides(...)` (shared with Step 14.2d) so the `sessionPath` branch skips real mux-pane construction. It also creates real empty temp files on disk for the `sessionPath` cases because the handler's `existsSync(params.sessionPath)` gate sits between the mux gate and the watcher — that gate is intentionally not seamed (it protects a downstream `getNewEntries` read).
...
**Files:**
- Modify: `pi-extension/subagents/backends/types.ts` — add `ping?`, `sessionKey?` to `BackendResult`; extend `Backend.watch(handle, signal?, onUpdate?, hooks?)` with an additive 4th-positional `hooks?: { onSessionKey?: (key: string) => void }` parameter — the existing `signal` (2nd) and `onUpdate` (3rd) parameters are preserved unchanged (Step 9.5b Part A).
- Modify: `pi-extension/subagents/backends/pane.ts` — forward the new `hooks` arg into `watchSubagent`; populate `BackendResult.ping` and `BackendResult.sessionKey`. Existing `signal` + `onUpdate` plumbing unchanged.
- Modify: `pi-extension/subagents/backends/headless.ts` — add `.exit` sidecar detection for pi (Step 9.5); fire `hooks.onSessionKey` the first time the `system/init` event is parsed in `runClaudeHeadless` (Step 9.5b Part C). Existing `signal` + `onUpdate` plumbing unchanged.
- Modify: `pi-extension/subagents/index.ts` — extend `watchSubagent(running, signal, opts?)` with an `onSessionKey` opt that fires from two sites for Claude children: a best-effort early read inside `pollForExit`'s `onTick` the first time the `.transcript` pointer file appears, AND a load-bearing synchronous read immediately before `watchSubagent` resolves its promise — the second site closes the race where a `caller_ping` arrives between polling ticks and `pollForExit` exits before the next `onTick` runs (Step 9.5b Part B).
- Modify: `pi-extension/orchestration/default-deps.ts` — forward the new `hooks` 4th-arg straight through from `LauncherDeps.waitForCompletion(handle, signal, onUpdate, hooks)` into `backend.watch(handle, signal, onUpdate, hooks)`. Existing `signal` + `onUpdate` projection preserved unchanged (Step 9.5b Part D).
- Modify: `pi-extension/orchestration/types.ts` — extend `LauncherDeps.waitForCompletion` with an additive 4th-positional `hooks?: { onSessionKey?: (sessionKey: string) => void }` parameter; the existing `signal` and `onUpdate` parameters are preserved unchanged.
- Modify: `pi-extension/orchestration/run-serial.ts`, `run-parallel.ts` — accept `onSessionKey` opt and append it as the new 4th argument to the existing `deps.waitForCompletion(handle, opts.signal, stepOnUpdate, ...)` call site; the first three arguments remain unchanged.
...
- Create: `test/orchestration/backend-seam.test.ts` — backend-seam contract: a fake backend that fires `hooks.onSessionKey` mid-run routes into `registry.updateSessionKey` via the full runner → dispatcher path; race-closer pre-return synchronous fire pinned (Step 9.5b Parts G); regression cases pin `signal`-aborts and `onUpdate`-partials preservation across the new signature (Step 9.5b Part H).
...
In `pi-extension/subagents/backends/pane.ts::watch`, accept the new `hooks` 4th parameter (added in Step 9.5b Part A), forward `hooks.onSessionKey` into `watchSubagent`, and populate `BackendResult.sessionKey` and `ping`. The existing 2nd-positional `signal` and 3rd-positional `onUpdate` parameters are preserved unchanged — `pane.ts::watch` should keep accepting and forwarding them exactly as today. Pi-backed children already know `sessionKey` at launch time (`running.sessionFile`); the mid-run callback is only meaningful for Claude-backed children, where the session id only becomes known once Claude writes its transcript pointer file. `watchSubagent`'s `onTick` is the concrete hook that observes that pointer file (see Step 9.5b).

```ts
async watch(
  handle: LaunchedHandle,
  signal?: AbortSignal,
  onUpdate?: (partial: BackendResult) => void, // PRESERVED — existing param
  hooks?: BackendWatchHooks,                    // NEW — additive 4th positional
): Promise<BackendResult> {
  // ...existing running lookup + abort wiring unchanged...
  // NOTE: onUpdate plumbing into watchSubagent is unchanged from today; if
  // pane.ts::watch already projects partials, that path stays as-is. The only
  // delta is forwarding hooks.onSessionKey into watchSubagent's opts.
  try {
...
`Backend.watch`'s pre-existing `signal?: AbortSignal` (2nd positional) and `onUpdate?: (partial: BackendResult) => void` (3rd positional) are **load-bearing today** and must be preserved unchanged — `default-deps.ts`, `headless.ts`, `pane.ts`, `run-serial.ts`, and `run-parallel.ts` all rely on both, and `test/orchestration/headless-onupdate-replay.test.ts` and `test/orchestration/headless-abort.test.ts` pin those behaviors. Add `onSessionKey` purely additively as a new 4th positional `hooks` parameter (a small object so future hooks can join without further signature churn):

```ts
export interface BackendWatchHooks {
  /**
   * Late-bound notification of the child's resume-addressable session key.
   * Fires at most once per call. Pi children populate this at launch and
   * fire from the backend on entry; Claude children fire on the
   * `system/init` event (headless) or from `watchSubagent`'s pre-return
   * synchronous read (pane). See Step 9.5b Parts B and C.
   */
...
Verify before editing: `grep -rn "backend\.watch\|Backend\.watch\|backend\.watch" pi-extension/` — there are existing call sites for both `signal` and `onUpdate`; the new `hooks` arg slots in alongside them as a 4th positional with no behavioral change to the prior three.

**Part B — extend `watchSubagent` with an `onSessionKey` opt** (`pi-extension/subagents/index.ts`):

The load-bearing guarantee this part must deliver: **by the time `watchSubagent` resolves its promise with any `SubagentResult`, if the Claude session id is derivable at all, `onSessionKey` has already been invoked synchronously.** This closes the race where a Claude child calls `caller_ping` between two 1000ms polling ticks: the `.transcript` pointer exists and the child has exited before `onTick` gets another chance to run. If the callback fires only from `onTick`, `registry.updateSessionKey` runs *after* the ping result has already propagated to `registry.onTaskBlocked`, and the blocked steer-back carries an unroutable `sessionKey`.

Use two fire sites for Claude. The `onTick` read is a best-effort early binding so long-running children populate ownership without waiting for exit. The **synchronous final read immediately before `watchSubagent` returns** is the race-closer: if `firedSessionKey === false` at the point the function is about to resolve, synchronously `existsSync` + `readFileSync` the pointer and fire the callback before the promise resolves. Because `opts.onSessionKey` chains synchronously down to `registry.updateSessionKey` (Parts D–F are all synchronous call-through), the ownership map is populated before the caller (`run-serial` / `run-parallel`) routes `result.ping` to `opts.onBlocked`.
...
export async function watchSubagent(
  ...
  // Synchronously derive the Claude session id from the .transcript pointer
  // file the Stop-hook plugin writes at system/init. Returns undefined when
  // the pointer doesn't exist yet or is unreadable. Cheap enough to call on
  // every tick AND one final time at exit.
  ...
  const result = await pollForExit(surface, AbortSignal.any([signal, getModuleAbortSignal()]), {
    ...
    onTick() {
      // Best-effort early binding: long-running Claude children populate
      // ownership within one polling interval of their system/init event.
      if (!firedSessionKey) maybeFire(readClaudeSessionId());
      // ...existing per-tick entry/byte accounting unchanged...
    },
  });

  // LOAD-BEARING race-closer. If a caller_ping fires between ticks, the next
  // onTick never runs because pollForExit resolves on sentinel exit. Read the
  // pointer synchronously here so onSessionKey — and therefore
  // registry.updateSessionKey — runs before this function's promise resolves
  // with a ping-carrying result. This call must happen on every exit path,
  // not only the ping path, so that a terminal-without-ping still updates
  // ownership for any external resume that races the exit.
  if (!firedSessionKey) maybeFire(readClaudeSessionId());
...
If the child exits before the `.transcript` pointer ever appears (launch failure path), both fire sites no-op and `sub.claudeSessionId` falls back to the existing post-exit archival path — no regression.

**Why this is race-free.** The runner site is (note: 1st–3rd args are unchanged from today's code, only the 4th `hooks` arg is new):

```ts
const result = await deps.waitForCompletion(handle, opts.signal, stepOnUpdate, {
  onSessionKey: (sessionKey) => opts.onSessionKey?.(i, sessionKey),
});
// ...then inspect result.ping and call opts.onBlocked(...) if set.
```

`deps.waitForCompletion` forwards into `backend.watch(handle, signal, onUpdate, hooks)` (Part D). `backend.watch` is `watchSubagent` through the pane adapter. `watchSubagent` invokes `hooks.onSessionKey` **synchronously** via `maybeFire` on its final pre-return line, before resolving the outer promise. That fire chains synchronously through `hooks.onSessionKey → deps.waitForCompletion's hooks param → runner's onSessionKey → tool-handler onSessionKey → registry.updateSessionKey`. Only after all of that does the `await` resolve, and only then does the runner read `result.ping` and call `opts.onBlocked → registry.onTaskBlocked`. So `registry.updateSessionKey` is guaranteed to run before `registry.onTaskBlocked` reads the ownership map, and the blocked steer-back carries the canonical `sessionKey`.

**Part C — fire `onSessionKey` from `runClaudeHeadless`** (`pi-extension/subagents/backends/headless.ts`): the headless backend's `watch(handle, signal?, onUpdate?, hooks?)` accepts the new `hooks` 4th argument and forwards `hooks?.onSessionKey` into `runClaudeHeadless`/`runPiHeadless` via their existing spec object. As with the pane adapter, the existing 2nd-positional `signal` and 3rd-positional `onUpdate` are preserved unchanged. In `runClaudeHeadless`, invoke the callback the first time the `system/init` event is parsed. For `runPiHeadless`, no mid-run binding is needed (session file path is already set at launch — fire `hooks?.onSessionKey?.(spec.subagentSessionFile)` once at the start of the run for symmetry).

**Part D — thread hooks through `LauncherDeps`** (`pi-extension/orchestration/types.ts`):

`LauncherDeps.waitForCompletion`'s pre-existing `signal?: AbortSignal` (2nd positional) and `onUpdate?: (partial: OrchestrationResult) => void` (3rd positional) are **load-bearing today** — `run-serial.ts:85` and `run-parallel.ts:88` both call `await deps.waitForCompletion(handle, opts.signal, stepOnUpdate)`, and `default-deps.ts` projects `BackendResult` partials into `OrchestrationResult` partials through the `onUpdate` callback. Both must remain functional after this change. Add `hooks` purely additively as a new 4th positional:

```ts
export interface WaitForCompletionHooks {
  /** Late-bound notification of the child's session key (Claude path). */
  onSessionKey?: (sessionKey: string) => void;
}
...
In `pi-extension/orchestration/default-deps.ts::waitForCompletion`, **preserve the existing `signal` + `onUpdate` projection unchanged** and forward the new `hooks` 4th-arg straight through to `backend.watch`. The whole change is one new parameter and one new call argument:
...
async waitForCompletion(
  ...
  const result = await backend.watch(
    ...
    onUpdate
      ? (partial) => {
          // existing BackendResult → OrchestrationResult projection — UNCHANGED
          onUpdate({
            name: partial.name,
            ...
            sessionKey: partial.sessionKey, // NEW (Task 9.6)
            error: partial.error,
            usage: partial.usage,
            transcript: partial.transcript,
            ping: partial.ping, // NEW (Task 9.6)
          });
        }
      : undefined,
    hooks, // NEW — additive 4th arg, contains onSessionKey
  );
  return { /* unchanged final projection (with sessionKey + ping additions from Task 9.6) */ };
}
```

**Part E — forward `onSessionKey` through runners** (`pi-extension/orchestration/run-serial.ts` / `run-parallel.ts`).

Extend `RunSerialOpts` / `RunParallelOpts` with `onSessionKey?: (taskIndex: number, sessionKey: string) => void`. **Do not change the existing `await deps.waitForCompletion(handle, opts.signal, stepOnUpdate)` call site's first three arguments** — abort propagation (`opts.signal`) and the per-task partial-update projection (`stepOnUpdate`) must continue to flow through unchanged. The change is: append the new `hooks` 4th argument with `onSessionKey` wired:

```ts
// run-serial.ts (line ~85) — BEFORE
result = await deps.waitForCompletion(handle, opts.signal, stepOnUpdate);

// run-serial.ts (line ~85) — AFTER (only the 4th arg is added)
result = await deps.waitForCompletion(handle, opts.signal, stepOnUpdate, {
  onSessionKey: (sessionKey) => opts.onSessionKey?.(i, sessionKey),
});
```

Apply the same delta to `run-parallel.ts` (line ~88). When `opts.onSessionKey` is undefined (sync `wait: true` path, where the registry is not involved), the inner closure no-ops — the call site behaves exactly as today.
...
// Helper: builds the tool harness (repeated across cases).
...
describe("backend-seam: mid-run onSessionKey routes into registry.updateSessionKey", () => {
  it("a Claude-backed fake that fires onSessionKey mid-run populates ownership before any blocked event", async () => {
    ...
    const deps: LauncherDeps = {
      ...
      // Match the post-9.5b signature: (handle, signal?, onUpdate?, hooks?).
      // The 1st-3rd args are preserved from today's contract; the new
      // hooks arg carries onSessionKey.
  ...
  it("race-closer: onSessionKey fired SYNCHRONOUSLY right before a ping-carrying result still lands in the registry before onTaskBlocked runs", async () => {
    // Worst case: Claude session id only becomes known at the exact moment
    // the child exits via caller_ping, so there is no further polling tick.
    // The backend must invoke hooks.onSessionKey synchronously in the same
    // stack frame that builds the ping-carrying return value — NOT via a
    // microtask / setTimeout — so registry.updateSessionKey lands before
    // the runner reads result.ping and routes to registry.onTaskBlocked.
    ...
    const deps: LauncherDeps = {
      ...
      async waitForCompletion(h, _signal, _onUpdate, hooks) {
        // Synchronous fire immediately before return — simulates
        // watchSubagent's load-bearing pre-return read.
        hooks?.onSessionKey?.(claudeId);
        return {
          name: h.name, finalMessage: "", transcriptPath: null,
...
The registry-level "late-bound sessionKey routes subsequent onTaskBlocked / onResumeTerminal" test added in Step 3.1 is still present and unchanged; the two cases above are the seam-level contract that proves the runner → dispatcher → `registry.updateSessionKey` wiring actually lands before the block. The second case ("race-closer") pins the synchronous-pre-return contract that Part B's final read delivers, so future edits cannot silently regress to a polling-only mechanism without breaking this test.

**Part H — preservation regression: `signal` and `onUpdate` still flow through the post-9.5b seam** (append to the same `test/orchestration/backend-seam.test.ts`):

The new `hooks` 4th argument must be additive only. Pin both the abort path and the partial-update path on the new signature so a future refactor cannot silently drop them. The two cases below assert directly against `LauncherDeps.waitForCompletion(handle, signal, onUpdate, hooks)`; the fake deps record what they receive. Together with the existing `test/orchestration/headless-onupdate-replay.test.ts` and `test/orchestration/headless-abort.test.ts` (which exercise the same behaviors at the backend layer), they form the regression net for Finding #1.

```ts
describe("backend-seam: signal + onUpdate preserved across the post-9.5b waitForCompletion signature", () => {
  it("opts.signal still aborts an in-flight waitForCompletion and the hooks arg does not break that path", async () => {
    const ac = new AbortController();
    ...
    const deps: LauncherDeps = {
      ...
      async waitForCompletion(h, signal, _onUpdate, hooks) {
        observedSignal = signal;
        // Best-effort fire to prove hooks still works alongside abort.
        hooks?.onSessionKey?.(`key-${h.name}`);
    ...
    const promise = serial.execute(
      "abort-1",
      { wait: false, tasks: [{ agent: "x", task: "t" }] },
      ac.signal, () => {}, { sessionManager: {} as any, cwd: "/tmp" },
    );
    await promise; // async dispatch returns immediately
    await new Promise((r) => setTimeout(r, 5));
    ac.abort();
    await new Promise((r) => setTimeout(r, 20));
    assert.ok(observedSignal, "waitForCompletion must still receive the AbortSignal as its 2nd positional arg");
    assert.equal(aborted, true, "abort propagation through opts.signal must still work after 9.5b");
  });
  ...
  it("onUpdate (3rd positional) still receives partials from the underlying watch", async () => {
    ...
    const deps: LauncherDeps = {
      ...
      async waitForCompletion(h, _signal, onUpdate, _hooks) {
        // Two partials, then terminal — proves the projection path is intact.
        onUpdate?.({
          name: h.name, finalMessage: "p1", transcriptPath: null,
          exitCode: 0, elapsedMs: 1,
        });
        onUpdate?.({
          name: h.name, finalMessage: "p2", transcriptPath: null,
          exitCode: 0, elapsedMs: 2,
        });
        return {
          name: h.name, finalMessage: "done", transcriptPath: null,
          exitCode: 0, elapsedMs: 3,
        };
      },
    };
    // Wire the tool framework's onUpdate so the runner's stepOnUpdate fires.
    const tools: any[] = [];
    const api = {
      registerTool: (t: any) => tools.push(t),
      on() {}, registerCommand() {}, registerMessageRenderer() {},
    ...
    await serial.execute(
      ...
      new AbortController().signal,
      (frame: any) => partials.push(frame),
      { sessionManager: {} as any, cwd: "/tmp" },
    );
    assert.ok(partials.length >= 1, "stepOnUpdate must still propagate per-task partials after 9.5b");
  });
});
```

If either case fails, the seam was edited in a way that dropped `signal` or `onUpdate` — revert and re-do the change additively.
...
onResumeTerminal(sessionKey, result) {
  ...
  if (!own) return;
  const entry = entries.get(own.orchestrationId);
  // The slot is either still `blocked` (onResumeStarted never fired — e.g. an
  // unseeded test harness or a race) or already `running` (normal case —
  // onResumeStarted transitioned it when the resume launch began). Both are
  // "was paused by a block" for sweep-decision purposes.
...
**Files:**
- Modify: `pi-extension/subagents/index.ts` (the `subagent_resume.execute` handler, plus `__test__.getRegistry` / `__test__.resetRegistry` seams for Step 12.2a)
- Modify: `test/integration/orchestration-async.test.ts` (add a resume e2e test) OR create a new test file
- Create: `test/orchestration/resume-tool-boundary.test.ts` (Step 12.2a)
...
Inside `execute`, the very first statement (ahead of any other gate, including `isMuxAvailable()`) validates that exactly one of `sessionPath` / `sessionId` is set and computes the canonical ownership key. The XOR check must precede the mux gate so callers see a parameter error regardless of mux availability — this ordering is what makes Step 12.2a's cases 1 and 2 deterministic without requiring a mux backend to be installed:

```ts
async execute(_id, params, _signal, _onUpdate, ctx) {
  // 1. XOR validation — must precede the mux gate (Step 12.2a depends on this).
  ...
  // 2. Mux gate (preserves existing behavior; honors __test__ override from Task 7b).
  // Test seam: Task 7b (__test__.setMuxAvailableOverride).
  const muxAvailable = muxAvailableOverride ?? isMuxAvailable();
  if (!muxAvailable) {
    return muxUnavailableResult();
  ...
  // 3. Branch on which identifier was provided (sessionPath = pi resume,
  //    sessionId = Claude resume). See Step 12.2 below for the launch
  //    construction for each branch.
  // ...
}
```

Branch the actual resume launch on which identifier was provided.

**`sessionPath` branch (pi resume — unchanged from today):** the existing mux-pane resume code path. Builds a `pi --session <sessionPath>` command (loading the `subagent-done.ts` extension), opens a mux pane via `createSurface` + `sendLongCommand`, registers a `RunningSubagent` with `backend: "pane"` and `sessionFile: params.sessionPath`. No `cli: "claude"` and no `sentinelFile`. `watchSubagent` follows its non-Claude path (the `running.cli !== "claude"` branch in `pi-extension/subagents/index.ts:854`), polling `running.sessionFile` for entry-count / byte updates.

**`sessionId` branch (Claude resume — NEW; mirrors the Claude launch path in `launchSubagent` at `pi-extension/subagents/index.ts:617`):** reuse the same Claude pane infrastructure that `launchSubagent` uses for fresh Claude children. Concretely:

1. **Allocate a sentinel.** Generate a fresh id (`Math.random().toString(16).slice(2, 10)`) and a sentinel path: `const sentinelFile = ` `` `/tmp/pi-claude-${id}-done` ``. The Stop-hook plugin writes the final-message file at `sentinelFile` and the Claude session-id pointer at `sentinelFile + ".transcript"`. These are exactly the files `watchSubagent`'s Claude branch reads (`pi-extension/subagents/index.ts:873–902`) and the same files Step 9.5b's `readClaudeSessionId()` polls.
2. **Locate the plugin dir.** `const pluginDir = join(dirname(new URL(import.meta.url).pathname), "plugin"); const pluginDirResolved = existsSync(pluginDir) ? pluginDir : undefined;` — same construction as the launch path. The plugin dir provides the Stop hook that emits `subagent_done` / `caller_ping` sentinels; without it, the resumed Claude child cannot signal completion or pings to `watchSubagent`.
3. **Build the argv with `buildClaudeCmdParts({ ..., resumeSessionId: params.sessionId, task: params.message ?? "" })`.** The existing helper already supports `resumeSessionId`; passing it produces the `claude --resume <sessionId>` invocation with the plugin loaded and the follow-up message delivered as the prompt body. Pass through `sentinelFile`, `pluginDir: pluginDirResolved`. Other fields (model, thinking, identity, systemPromptMode, effectiveTools) come from a minimal spec — for resume there is no agent definition; pass `undefined` / sensible defaults so the helper falls back to whatever the resumed session itself was configured with. (Claude `--resume` re-hydrates the prior model/system from the session file, so most fields are no-ops on the resume path.)
4. **Open the pane and dispatch.** Same `createSurface(name, { detach: false })` + `getShellReadyDelayMs()` wait + `sendLongCommand(surface, command, ...)` pattern the `sessionPath` branch already uses. The wrapping `cd ${cwd} && <argv>; echo '__SUBAGENT_DONE_'$?'__'` shape is identical (so `pollForExit`'s exit-marker detection works unchanged).
5. **Register a Claude-shaped `RunningSubagent`** so `watchSubagent`'s Claude branch handles completion correctly:

```ts
const running: RunningSubagent = {
  id,
  name: params.name ?? "Resume",
  task: params.message ?? "resumed Claude session",
  backend: "pane",
  surface,
  startTime,
  // For Claude, sessionFile is unused by the watcher (the Claude branch reads
  // sentinelFile + ".transcript" instead). Set to "" or omit per RunningSubagent's
  // optional contract.
  sessionFile: "",
  launchScriptFile,
  cli: "claude",
...
6. **Watcher invocation is shared.** Call the same `watchSubagent(running, watcherAbort.signal, opts)` the `sessionPath` branch calls. The Claude-vs-pi dispatch happens inside `watchSubagent` based on `running.cli`. Step 9.5b's `onSessionKey` hook fires the same way for the resumed child — Step 9.5b Part B's pre-return synchronous read of `sentinelFile + ".transcript"` derives the resumed Claude session id (which equals the input `params.sessionId` after Claude rotates it; record whichever Claude reports) and surfaces it through `opts.onSessionKey`.

**Resume-start lifecycle transition (`blocked -> running`).** Immediately after the pane is launched and the `RunningSubagent` is registered (common to both the `sessionPath` and `sessionId` branches), and before `watchSubagent(...)` is awaited, call `registry.onResumeStarted(sessionKey)`. This is the spec's `blocked -> running` leg for owned slots:

- For orch-owned sessions this transitions the registry snapshot's slot back to `running` (from `blocked`) and fires the `RegistryHooks.onResumeStarted` extension hook, which Task 13.4 uses to drop the virtual blocked widget row at resume-start time (so the now-visible real `RunningSubagent` pane does not double up with a stale virtual row).
- For unowned sessions (the common standalone-resume case) it is a no-op — `lookupOwner` returns `null` and the registry returns early.

Place the call right after `runningSubagents.set(id, running); startWidgetRefresh();`, so the widget refresh sees both the removal of the virtual row and the arrival of the real row on the same tick:

```ts
// Just after the RunningSubagent is registered and startWidgetRefresh() fires:
registry.onResumeStarted(sessionKey);
// The watcher kickoff that follows is unchanged.
```

**Compute the canonical `sessionKey` for registry routing.** For both branches, set `const sessionKey = params.sessionPath ?? params.sessionId!;` (already established above) and pass it through to the registry routing block below. The Claude branch's resume re-launch may produce a *different* downstream Claude session id when the child eventually rotates the session file; the ownership map stays keyed on the original `params.sessionId` because that is what the parent passed to `subagent_resume` and what `registry.lookupOwner` will be queried with. If the watcher's `onSessionKey` later surfaces a rotated id, route ownership transfer through `registry.updateSessionKey` so subsequent recursive resumes can target the new id — but that flow is out of scope for v1; document the gap with a one-line code comment and leave it for a follow-up if real usage needs it.

**No-mux fallback.** Both branches require a multiplexer (the `isMuxAvailable()` gate already short-circuits at the top of `execute` per Step 12.2 above). The error message returned by `muxUnavailableResult()` is identical for both branches.
...
- [ ] **Step 12.2a: Tool-boundary tests for `subagent_resume({ sessionPath | sessionId })`**

**Files:**
- Create: `test/orchestration/resume-tool-boundary.test.ts`

The Step 12.1 tests exercise the registry-level re-ingestion contract. The Step 14.2 tests exercise recursion/terminal routing through the real extension wiring against a seamed `watchSubagent`. Neither directly asserts the **tool-parameter boundary** the Step 12.2 schema change introduces: exactly one of `sessionPath` / `sessionId` must be provided, and both must route through the same `registry.lookupOwner(sessionKey)` lookup regardless of which was passed. Without this coverage it is easy to regress either the XOR validation or the `sessionId`-path dispatch silently while still passing every other test in this plan.

Write a focused unit-level test against the registered `subagent_resume` tool's `execute` handler. The test uses four `__test__` seams — three from Task 7b plus `setSurfaceOverrides` from Step 14.2d:

- `setWatchSubagentOverride(...)` — so the test controls the resumed-session outcome without spawning a real child.
- `getRegistry` / `resetRegistry` — so the test can seed ownership state and isolate cases.
- **`setMuxAvailableOverride(true)`** — so the test reaches the registry-routing branches on no-mux hosts. Without this, `subagent_resume.execute` short-circuits at the `isMuxAvailable()` gate and cases 3–6 never invoke the watcher override. This is the seam introduced in Task 7b's Step 7b.4 specifically to make this boundary test executable on no-mux machines (CI runners, headless dev hosts).
- **`setSurfaceOverrides({ createSurface, sendLongCommand })`** — so the real mux-pane construction is stubbed. The `sessionPath` branch of the real handler calls `createSurface(...)` + `sendLongCommand(...)` (via the same path `launchSubagent` uses); without stubbing them, the test would attempt to open a real mux pane before ever reaching `watchSubagent`. This is the same seam Step 14.2d uses for the Claude branch; the `sessionPath` cases here need it for the identical reason.

**`sessionPath` existence gate.** Additionally — and this is the point v9's sessionPath cases missed — the real handler has an `existsSync(params.sessionPath)` check sitting **between** the mux gate and the watcher invocation (see `pi-extension/subagents/index.ts:1413`). If the seeded ownership path does not exist on disk, the handler returns early with `{ error: "session not found" }` and the watcher override is never called. There is no test seam for this gate — it is guarding a real filesystem invariant the resume code depends on downstream (entry-count diffing via `getNewEntries(params.sessionPath, 0)` at the same site). The test therefore **creates real empty temp files for every `sessionPath` case** (cases 3 and 6) in `beforeEach`, and unlinks them in `afterEach`. Empty files are fine: `getNewEntries` handles an empty file as zero entries. Use `fs.mkdtempSync(path.join(os.tmpdir(), "resume-boundary-"))` to create an isolated scratch directory per `describe` run so cases do not collide.

The XOR validation cases (1 and 2) deliberately do NOT depend on the mux override **or** the temp-file setup — the XOR check must happen *before* the mux gate and the existsSync gate so callers see the parameter error regardless of mux availability or session-file presence. Pin that ordering at the implementation site (Step 12.2 — the XOR check is the very first statement in `execute`, ahead of `isMuxAvailable()` which is itself ahead of `existsSync(params.sessionPath)`).

The test does NOT require `pi` to be on PATH and does NOT require a real mux backend — both are intentionally bypassed via the override. The test is about the tool parameter contract and the registry dispatch, not the backend launch.

The test must cover each of the following cases:

1. **XOR validation — neither provided.** Invoke `subagent_resume.execute({ message: "hi" }, ...)` (no `sessionPath`, no `sessionId`). Assert the handler returns `{ isError: true }` with text content that names both `sessionPath` and `sessionId`. `watchSubagentOverride` must NOT be called (i.e., no resume launched).
2. **XOR validation — both provided.** Invoke with `{ sessionPath: "/tmp/a.jsonl", sessionId: "claude-x", message: "hi" }`. Assert `{ isError: true }` identical to case 1; no launch.
3. **`sessionPath` path routes to registry.** Seed the registry with an orchestration whose task 0 is blocked on `ownedPath = join(scratchDir, "owned.jsonl")`, **where `ownedPath` was created as an empty file in `beforeEach`** (so it passes the `existsSync` gate). Stub `watchSubagentOverride` to return a terminal (non-ping) result. Invoke `subagent_resume.execute({ sessionPath: ownedPath, message: "go" }, ...)`. Assert (a) the stubbed watcher was invoked, (b) `registry.getSnapshot(orchId).tasks[0].state === "completed"` after the resume settles, (c) the aggregated `orchestration_complete` steer-back fired via the fake pi's `sendMessage`.
4. **`sessionId` path routes to registry.** Seed the registry with a Claude-backed block: `registry.onTaskLaunched(id, 0, {})` then `registry.updateSessionKey(id, 0, claudeId)` then `registry.onTaskBlocked(id, 0, { sessionKey: claudeId, ... })`. Stub `watchSubagentOverride` to return a terminal result. Invoke `subagent_resume.execute({ sessionId: claudeId, message: "go" }, ...)`. Assert the same three things as case 3, keyed on `claudeId`. No temp file needed — the `sessionId` branch does not hit the `existsSync` gate.
5. **`sessionId` path with ping-during-resume triggers `onTaskBlocked` recursion.** Same seed as case 4; stub the watcher to return a `ping`-carrying result. Assert a `BLOCKED_KIND` steer-back fires with `details.sessionKey === claudeId`, not a terminal completion. No temp file needed (sessionId branch).
6. **Unowned `sessionPath` still resumes cleanly.** Use `strayPath = join(scratchDir, "stray.jsonl")`, **also created as an empty file in `beforeEach`**, so the handler gets past the existsSync gate and reaches `registry.lookupOwner` (which returns `null` — the test assertion). Do NOT seed any registry ownership for that path. Invoke `subagent_resume.execute({ sessionPath: strayPath, message: "go" }, ...)`. Assert the handler returns without error and `registry.listActive()` is unchanged (no orchestration was mutated). This is the regression-guard that `registry.lookupOwner` returning `null` does not crash the handler for standalone (non-orch) resumes — the most common production case.
...
describe("subagent_resume tool boundary", () => {
  ...
  let tools: any[];
  let registry: any;           // captured from __test__ in each case
  let scratchDir: string;      // holds the real session files the sessionPath cases need
  let ownedPath: string;       // seeded as orchestration-owned for case 3
  let strayPath: string;       // unowned for case 6
  ...
  beforeEach(() => {
    ...
    registry = __test__.getRegistry(); // expose via __test__ (Task 7b)
    // Mux gate: cases 3–6 below need to reach the post-mux registry routing
    // even on hosts where no terminal multiplexer is installed. Force the
    // gate open via the Task 7b override; the watcher itself is overridden
    // separately so no real mux is ever touched.
    __test__.setMuxAvailableOverride(true);
    // Surface seam (from Step 14.2d): stub createSurface + sendLongCommand so
    // the sessionPath branch's real mux-pane construction is skipped. The
    // handler still reaches watchSubagent — which we override below.
    __test__.setSurfaceOverrides?.({
      createSurface: () => "test-surface",
      sendLongCommand: () => {},
    });
    // existsSync gate (real-handler invariant — see Step 12.2 preamble):
    // the sessionPath branch has an `existsSync(params.sessionPath)` guard
    // between the mux gate and the watcher. Create real empty files for the
    // two paths used by cases 3 and 6 so the handler gets past that guard
    // and actually invokes watchSubagentOverride. Cases 4 and 5 use sessionId
    // and never hit this guard.
  ...
  function getResume() {
    const t = tools.find((x) => x.name === "subagent_resume");
    assert.ok(t, "subagent_resume must be registered");
    return t;
  }
  ...
  it("rejects when neither sessionPath nor sessionId is provided", async () => {
    let watcherCalled = false;
    __test__.setWatchSubagentOverride(async () => { watcherCalled = true; return null as any; });
    const out = await getResume().execute("c1", { message: "hi" },
      new AbortController().signal, () => {}, toolCtx());
    assert.equal(out.isError, true);
    const text = out.content?.find((c: any) => c.type === "text")?.text ?? "";
    assert.match(text, /sessionPath/);
    assert.match(text, /sessionId/);
    assert.equal(watcherCalled, false);
  });

  it("rejects when both sessionPath and sessionId are provided", async () => {
    let watcherCalled = false;
    __test__.setWatchSubagentOverride(async () => { watcherCalled = true; return null as any; });
    const out = await getResume().execute("c2",
      { sessionPath: "/tmp/a.jsonl", sessionId: "claude-x", message: "hi" },
      new AbortController().signal, () => {}, toolCtx());
    assert.equal(out.isError, true);
    assert.equal(watcherCalled, false);
  });

  it("sessionPath path routes terminal resume through registry.onResumeTerminal", async () => {
    // ownedPath was created as an empty real file in beforeEach so it passes
    // the handler's existsSync gate and reaches the registry routing.
    const id = registry.dispatchAsync({
      config: { mode: "serial", tasks: [{ name: "a", agent: "x", task: "t" }] },
    });
    registry.onTaskLaunched(id, 0, { sessionKey: ownedPath });
    registry.onTaskBlocked(id, 0, { sessionKey: ownedPath, message: "?" });
    __test__.setWatchSubagentOverride(async () => ({
      // terminal, no ping — shape matches what subagent_resume's watcher consumes
      exitCode: 0, error: undefined, ping: undefined, elapsed: 0.01,
      // ...remaining fields per SubagentResult shape
    }) as any);
    await getResume().execute("c3", { sessionPath: ownedPath, message: "go" },
      new AbortController().signal, () => {}, toolCtx());
    await new Promise((r) => setTimeout(r, 20));
    assert.equal(registry.getSnapshot(id).tasks[0].state, "completed");
    const complete = sentMessages.find((m) =>
      m.details?.kind === ORCHESTRATION_COMPLETE_KIND,
    );
    assert.ok(complete, "orchestration_complete must fire after resume");
  });

  it("sessionId path routes terminal resume through registry.onResumeTerminal", async () => {
    const claudeId = "claude-sess-boundary";
    const id = registry.dispatchAsync({
      config: { mode: "serial", tasks: [{ name: "a", agent: "x", task: "t" }] },
    });
    ...
    await getResume().execute("c4", { sessionId: claudeId, message: "go" },
      new AbortController().signal, () => {}, toolCtx());
    await new Promise((r) => setTimeout(r, 20));
    assert.equal(registry.getSnapshot(id).tasks[0].state, "completed");
    const complete = sentMessages.find((m) =>
  ...
  it("sessionId path with ping-during-resume routes through registry.onTaskBlocked", async () => {
    const claudeId = "claude-sess-recursive";
    const id = registry.dispatchAsync({
      config: { mode: "serial", tasks: [{ name: "a", agent: "x", task: "t" }] },
    });
    registry.onTaskLaunched(id, 0, {});
    registry.updateSessionKey(id, 0, claudeId);
    registry.onTaskBlocked(id, 0, { sessionKey: claudeId, message: "?" });
    sentMessages.length = 0; // ignore the first blocked emission
    __test__.setWatchSubagentOverride(async () => ({
      exitCode: 0, error: undefined,
      ping: { name: "a", message: "still need input" },
      elapsed: 0.01,
    }) as any);
    await getResume().execute("c5", { sessionId: claudeId, message: "go" },
      new AbortController().signal, () => {}, toolCtx());
    await new Promise((r) => setTimeout(r, 20));
    const reBlocked = sentMessages.find((m) => m.details?.kind === BLOCKED_KIND);
    assert.ok(reBlocked);
    assert.equal(reBlocked.details.sessionKey, claudeId);
  });

  it("unowned sessionPath resumes cleanly (standalone non-orch path) without mutating registry", async () => {
    // strayPath was created as an empty real file in beforeEach so the handler
    // passes existsSync and reaches registry.lookupOwner (which returns null —
    // the point of the test). Without the real file the handler would return
    // "session not found" before this assertion could run.
    __test__.setWatchSubagentOverride(async () => ({
      exitCode: 0, error: undefined, ping: undefined, elapsed: 0.01,
    }) as any);
    const activeBefore = registry.listActive().length;
    const out = await getResume().execute("c6",
      { sessionPath: strayPath, message: "go" },
      new AbortController().signal, () => {}, toolCtx());
    assert.notEqual(out.isError, true);
    assert.equal(registry.listActive().length, activeBefore);
  });
});
```

**Implementation note.** This test requires two small extensions to `pi-extension/subagents/index.ts::__test__`:

- `getRegistry(): Registry` — exposes the internal registry instance so the test can seed ownership and read snapshots.
- `resetRegistry?(): void` — resets the internal registry so tests don't leak state across cases (optional; tests can also use fresh orchestration ids).

Add both alongside the existing `setLauncherDepsOverride` / `setWatchSubagentOverride` seams from Task 7b. Keep the additions inside the `__test__` namespace so the production surface is unchanged.

The `sessionPath` cases additionally consume `setSurfaceOverrides` — the seam introduced in Step 14.2d for the Claude-branch test. Land it in Task 7b alongside the other seams (the full set is enumerated in the Step 14.2d "Implementation notes for the new `__test__` surface members" block below) rather than splitting the seam's introduction across tasks. The production behavior change is zero — the overrides default to `null`, in which case the real `createSurface` / `sendLongCommand` run.

There is intentionally **no** test seam for the `existsSync(params.sessionPath)` gate: the tests create real empty files on disk instead. Adding a seam here would let tests silently bypass a load-bearing production invariant (the handler's downstream `getNewEntries(params.sessionPath, 0)` call at the same site requires the file to be readable).

Run: `node --test test/orchestration/resume-tool-boundary.test.ts`
Expected: PASS (once Step 12.2 and the two `__test__` extensions above are in place).
...
Run: `node --test test/orchestration/registry.test.ts test/orchestration/async-dispatch.test.ts test/orchestration/block-resume.test.ts test/orchestration/resume-tool-boundary.test.ts test/integration/orchestration-async.test.ts`
Expected: PASS.
...
Extend the registry wiring from Task 5 with three cleanup paths for the virtual blocked row:

1. **On `BLOCKED_KIND` emission** — install a virtual `RunningSubagent` row keyed on `(orchestrationId, taskIndex)` so the widget keeps showing the blocked state after the child's own pane closes.
2. **On `RegistryHooks.onResumeStarted` (NEW)** — drop the virtual row the instant a standalone `subagent_resume` starts on the owning sessionKey. This is the spec's `blocked -> running` leg at the widget layer: `subagent_resume` registers a real `RunningSubagent` for the resumed pane, so the virtual row must go away on the same tick to avoid a double row for the same task.
3. **On `RegistryHooks.onTaskTerminal`** — drop the virtual row on any per-task terminal transition (completed / failed / cancelled). This still handles the "blocked task is cancelled without ever being resumed" case that `onResumeStarted` does not cover.

Declare the virtual-blocked map and the `onOrchestrationTaskTerminal` stub introduced in Task 5, add a matching `onOrchestrationResumeStarted` helper, then replace the emitter body:
...
function onOrchestrationResumeStarted(orchestrationId: string, taskIndex: number): void {
  // Spec: blocked -> running on resume. Remove the virtual row so the real
  // resumed-pane RunningSubagent (registered by subagent_resume just before
  // it calls registry.onResumeStarted) is the only row shown for this task.
  dropVirtualBlocked(orchestrationId, taskIndex);
}
...
const registry = createRegistry(
  ...
  },
  {
    ...
    // orchestration to complete.
    onTaskTerminal: ({ orchestrationId, taskIndex }) => {
      onOrchestrationTaskTerminal(orchestrationId, taskIndex);
    },
    // Resume-start transitions (blocked -> running) clear the virtual row at
    // resume-start time, not at terminal time — otherwise a resumed pane's
    // real RunningSubagent row would coexist with the stale virtual row until
    // the resumed task completes. This is what implements the spec's
    // `blocked -> running` lifecycle leg at the widget layer.
    onResumeStarted: ({ orchestrationId, taskIndex }) => {
      onOrchestrationResumeStarted(orchestrationId, taskIndex);
    },
  },
...
(This replaces the registry-wiring block installed in Task 5 — Task 5 left the branches and the `onOrchestrationTaskTerminal` stub in place so that Task 13 can fill them in without re-plumbing the emitter contract. Task 13 additionally introduces the `onOrchestrationResumeStarted` helper and the matching `onResumeStarted` hook wiring.)

Also append two tests to `test/test.ts`:
...
it("removes the virtual blocked row the moment subagent_resume starts (blocked -> running)", () => {
  // Invariant: when registry.onResumeStarted(sessionKey) fires for an owned
  // slot, the corresponding `virt-${oid}:${idx}` entry is removed from
  // runningSubagents BEFORE the resumed task reaches a terminal state. This
  // is the spec's blocked -> running lifecycle leg at the widget layer.
  //
  // Test shape (use the existing __test__ surface):
  //   1. Boot subagentsExtension(fakePi) and grab the registry via
  //      __test__.getRegistry().
  //   2. registry.onTaskLaunched(id, 0, { sessionKey: "sess-a" });
  //   3. registry.onTaskBlocked(id, 0, { sessionKey: "sess-a", message: "?" });
  //   4. Assert runningSubagents has a `virt-${id}:0` entry.
  //   5. registry.onResumeStarted("sess-a");
  //   6. Assert runningSubagents no longer has `virt-${id}:0`.
  //   7. Assert registry.getSnapshot(id).tasks[0].state === "running".
});
```

Fill both test bodies using the same `__test__` surface already used by the existing widget tests.
...
- [ ] **Step 14.2d: Coverage for the Claude `sessionId` resume branch (non-seamed)**

**Files:**
- Create: `test/orchestration/claude-resume-branch.test.ts`

Steps 14.2b and 14.2c exercise the pi-backed `sessionPath` resume branch end-to-end. The Claude-backed `sessionId` branch added in Step 12.2 also needs at least one test that hits the **actual production code path** rather than only the watcher-override boundary cases in Step 12.2a. A real Claude-backed end-to-end test would require `claude` on PATH plus a Claude-compatible fixture agent — that infrastructure is not required by this plan and is deferred. Instead, add a focused unit-level test that exercises the real `subagent_resume.execute` Claude branch through the surface boundary, asserting on the `RunningSubagent` it constructs.

Concretely, this test pins three properties of the production code that the watcher-override path in Step 12.2a cannot reach:

1. The Claude branch reuses `buildClaudeCmdParts({ ..., resumeSessionId: params.sessionId })` (so the argv carries `--resume <sessionId>` exactly).
2. The constructed `RunningSubagent` has `cli: "claude"` and a `sentinelFile` of the form `/tmp/pi-claude-<id>-done` (so `watchSubagent`'s Claude branch handles completion correctly — see `pi-extension/subagents/index.ts:869`).
3. The `sessionKey` used for `registry.lookupOwner` is the input `params.sessionId` (so cross-orch re-ingestion routes correctly even when Claude later rotates its session id mid-resume).

Use the same `__test__` seams as Step 12.2a, plus one additional surface-layer fake to avoid requiring a real mux pane. The test does NOT stub `watchSubagent` — instead it inspects the `runningSubagents` map immediately after `execute` returns:
...
describe("subagent_resume Claude sessionId branch (non-seamed construction)", () => {
  ...
  beforeEach(() => {
    ...
    __test__.setMuxAvailableOverride(true);
    // Surface seam: stub createSurface + sendLongCommand so no real mux pane
    // is opened. The Claude branch's RunningSubagent is still constructed and
    // registered — that is what this test asserts on.
    __test__.setSurfaceOverrides?.({
      createSurface: () => "test-surface",
      sendLongCommand: () => {},
    });
    // Watcher seam: prevent the fire-and-forget watchSubagent call from
    // touching real files. The test does not assert on watcher behavior here
    // (Step 12.2a covers that); it only asserts on the constructed
    // RunningSubagent before the watcher runs.
    __test__.setWatchSubagentOverride(async () => ({
      exitCode: 0, error: undefined, ping: undefined, elapsed: 0,
    }) as any);
  });
  afterEach(() => {
    __test__.setMuxAvailableOverride(null);
  ...
  it("constructs a Claude-shaped RunningSubagent with cli='claude', a sentinelFile, and a --resume argv", async () => {
    ...
    await resume.execute("c-claude", { sessionId: claudeId, message: "go" },
      new AbortController().signal, () => {}, { sessionManager: {} as any, cwd: "/tmp" });
    // Inspect the most recently registered RunningSubagent.
    const entries = [...__test__.getRunningSubagents().values()];
    const last = entries[entries.length - 1];
    assert.equal(last.cli, "claude");
    assert.ok(last.sentinelFile && /^\/tmp\/pi-claude-[0-9a-f]+-done$/.test(last.sentinelFile));
    // The launch script (written by sendLongCommand) should reference --resume <claudeId>.
    // If the surface override captures the command, assert directly; otherwise
    // assert on launchScriptFile contents if it was actually written, or rely
    // on the registered argv recorded by the surface stub.
    assert.ok(__test__.getLastLaunchCommand?.()?.includes(`--resume ${claudeId}`));
  });
  ...
  it("registry ownership routes through the input sessionId, not any later-rotated id", async () => {
    ...
    const resume = tools.find((t) => t.name === "subagent_resume")!;
    await resume.execute("c-claude2", { sessionId: claudeId, message: "go" },
      new AbortController().signal, () => {}, { sessionManager: {} as any, cwd: "/tmp" });
    await new Promise((r) => setTimeout(r, 20));
    // The terminal watcher result was a clean exit — registry.onResumeTerminal
    // must have fired keyed on the input sessionId.
    assert.equal(registry.getSnapshot(orchId).tasks[0].state, "completed");
  });
});
```

**Implementation notes for the new `__test__` surface members.** This test introduces three small, scoped additions to the `__test__` object alongside the seams from Task 7b:

- `setSurfaceOverrides(overrides | null)` — swaps `createSurface` / `sendLongCommand` for the duration of a test so no real mux pane is opened. Production code path is unchanged (overrides default to `null`).
- `getRunningSubagents()` — exposes the internal `runningSubagents` map for inspection. Read-only intent; tests should not mutate.
- `getLastLaunchCommand()` — captures the most recent argv string sent through the surface override so tests can pin `--resume <id>` without parsing files.

Land these in the same Task 7b extension block alongside `setLauncherDepsOverride` / `setWatchSubagentOverride` / `setMuxAvailableOverride` / `getRegistry` / `resetRegistry`. Add a one-line contract test in `test/test.ts` (next to the existing `__test__` contract tests) verifying each new member has the expected type. If, during implementation, any of these prove unworkable (e.g. `createSurface` is not easily injectable without a deeper refactor), drop Step 14.2d to a documented follow-up instead of bending the production code further — the watcher-override coverage in Step 12.2a is sufficient to ship Phase 2 safely; this step is the additional defense-in-depth the v8 review requested for the Claude branch.

Run: `node --test test/orchestration/claude-resume-branch.test.ts`
Expected: PASS.
...
- Tool renames → Task 4
- Extension test seams (`__test__.setLauncherDepsOverride`, `__test__.setWatchSubagentOverride`, `__test__.setMuxAvailableOverride`, `__test__.getRegistry`/`resetRegistry`, `__test__.setSurfaceOverrides`/`getRunningSubagents`/`getLastLaunchCommand`) → Task 7b + Step 14.2d additions
...
- Claude-backed `sessionKey` late-binding via the additive `Backend.watch` `hooks.onSessionKey` parameter → Step 9.5b (Parts A–G), including backend-seam contract test (`test/orchestration/backend-seam.test.ts`); Part H regression-pins `signal` and `onUpdate` preservation across the new signature
- `subagent_resume` accepts `sessionPath` OR `sessionId` → Step 12.2 (wiring, including the concrete recipe for the Claude `sessionId` resume launch — sentinel allocation, plugin dir, `buildClaudeCmdParts` reuse, Claude-shaped `RunningSubagent`) + Step 12.2a (tool-boundary tests covering XOR validation + both-path routing, executable on no-mux hosts via `__test__.setMuxAvailableOverride(true)`) + Step 14.2d (non-seamed coverage of the Claude `sessionId` branch's construction)
...
- Backend-real Phase 2 async/block coverage (pane + headless) → Tasks 14.2b, 14.2c; Claude `sessionId` branch construction → Step 14.2d
