# Mux-Free Execution Implementation Plan (v8)
...
**What's new in v8 (vs. v7):** the two remaining moderate findings from `.pi/plans/reviews/2026-04-20-mux-free-execution-design-review-v9.md` are remediated — see the "v8 changelog" section at the bottom of this document for the full list. In summary:
...
- `test/integration/pane-claude-tool-restriction.test.ts` — **new in v6 (review-v7 additional note 2); v8 (review-v9 finding 1) switches its oracle from "assistant text contains marker" to "filesystem side-effect file exists with expected contents"**, so refusal prose and hallucinated marker text cannot produce false pass/fail outcomes on this security-sensitive boundary. Exercises the full pane command-string path (not just the argv array) so the `--tools` emission is verified end-to-end. Skips when `claude` is not on PATH.
...
/**
 * Boundary projection from pi-ai `Message` onto `TranscriptMessage`
 * (v7, review-v8 finding 1).
 *
 * v6 pushed pi `Message` objects through an `as unknown as TranscriptMessage`
 ...
 * `TranscriptMessage` fields, normalizes `UserMessage.content: string` into a
 ...
 * already satisfy the shape (content is always an array of blocks in pi-ai's
 * `AssistantMessage`) so they pass through without content rewriting.
 *
 ...
 */
export function projectPiMessageToTranscript(msg: Message): TranscriptMessage {
  const rawContent: unknown = (msg as any).content;
  ...
  return new Promise<BackendResult>((resolve) => {
    ...
    const processLine = (line: string) => {
      ...
      if (event.type === "message_end" && event.message) {
        ...
        // instead. This keeps the projection focused on the public shape.
        if (msg.role === "assistant") {
          usage.turns++;
        ...
        transcript.push(projectPiMessageToTranscript(event.message as Message));
      }
    };
...
Expected: PASS on both cases if Claude is on PATH; skipped otherwise. A regression of review-v7 finding 1 manifests as case 1 failing because Bash actually wrote the marker file. A regression of review-v8 finding 2 manifests as case 2 hanging up to the test timeout (because `--print` becomes a positional, non-interactive mode is not engaged, and Claude sits in interactive mode waiting for more input). A regression of review-v9 finding 1 (reverting to a prose-based oracle) is caught at code review: this test must assert on `existsSync` / `readFileSync`, never on `stdout.includes(...)`.
