...
Store the callback on the `OrchestrationEntry`. Refine `onResumeTerminal` (introduced in Task 3) to invoke the continuation when the resumed slot was blocked:
...
```bash
git add README.md test/integration/orchestration-block-resume-e2e.test.ts test/integration/orchestration-extension-resume-routing.test.ts
