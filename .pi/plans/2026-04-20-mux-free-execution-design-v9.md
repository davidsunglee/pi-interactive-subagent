# Mux-Free Execution Implementation Plan (v9)
...
**What's new in v9 (vs. v8):** the two remaining moderate findings from `.pi/plans/reviews/2026-04-20-mux-free-execution-design-review-v10.md` are remediated — see the "v9 changelog" section at the bottom of this document for the full list. In summary:

1. **The headless pi backend no longer imports `Message` directly from `@mariozechner/pi-ai` (review-v10 finding 1, moderate).** v8 said "`@mariozechner/pi-ai` is no longer a direct dependency" (the dep addition v5 introduced was removed in v6) but Task 11 still added `import type { Message } from "@mariozechner/pi-ai"` to `headless.ts` and typed `projectPiMessageToTranscript(msg: Message)` + `event.message as Message` against that symbol. Importing a type from a package that is not declared as a direct dependency only resolves under transitive hoisting through `@mariozechner/pi-coding-agent`; that is not a stable contract — a future package-manager layout change, version skew, or peer-dep reshuffle can silently break the build. v9 replaces the pi-ai type import with a local minimal `PiStreamMessage` type defined next to the parser boundary in `headless.ts`. The type captures only what `projectPiMessageToTranscript` and the usage/stopReason accumulator actually read off the raw `message_end` / `tool_result_end` payloads, so the public boundary (`TranscriptMessage`) stays decoupled from pi-ai's internal `Message` union shape and `package.json` stays as it is in v8 (no direct `@mariozechner/pi-ai` dependency, no transitive-hoist assumption).

2. **The Claude-skills warning is now emitted on the pane-Claude path too (review-v10 finding 2, moderate).** v8 only emitted the "ignoring skills" warning inside `runClaudeHeadless` (Task 19 Step 2), so a user launching `cli: "claude"` through the existing pane path would still get a silent drop of `skills:`. The architecture section and README both said the limitation applied to "the Claude backend — pane **and** headless", so a pane-only silent-drop was the exact confusing behavior the plan said it wanted to avoid. v9 adds a new Step 3a inside Task 17 that threads the same warning into `launchSubagent()`'s Claude branch immediately after `effectiveSkills` is resolved but before `buildClaudeCmdParts` is invoked, using a shared `warnClaudeSkillsDropped(name, effectiveSkills)` helper in `index.ts` (also called by `runClaudeHeadless`) so the pane and headless call sites print exactly the same wording. A new unit test (`test/orchestration/claude-skills-warning.test.ts`) captures stderr around a direct `warnClaudeSkillsDropped(...)` invocation, and Task 17 Step 5's integration test gains a second describe block that asserts the stderr warning fires when a pane-Claude launch is constructed with `effectiveSkills` set (no model invocation needed — the warning fires at command-build time, before the child spawns).

**What was new in v8 (vs. v7):** the two remaining moderate findings from `.pi/plans/reviews/2026-04-20-mux-free-execution-design-review-v9.md` were remediated — see the "v8 changelog" section at the bottom of this document for the full list. In summary:

1. **The pane-Claude E2E restriction test in Task 17 Step 5 now asserts on a filesystem side effect, not assistant prose (review-v9 finding 1, moderate).** v7 keyed both cases off whether Claude's assistant text contained the literal marker `HELLO_FROM_BASH_42`. That is not a trustworthy oracle for a security-sensitive test: a compliant refusal can still quote the marker text (false failure in the restricted case), and the model can emit the marker text without actually invoking Bash (false pass in the unrestricted case). v8 rewrites both cases to ask Bash to `echo <marker> > <uniqueFile>` in a temp directory, and asserts on whether `<uniqueFile>` exists on disk — the only way the file can exist is if Bash actually ran. The restricted case asserts the file does **not** exist; the baseline case asserts the file exists and its contents equal the marker. Refusal prose and hallucinated marker text can no longer produce false pass/fail outcomes.

2. **Task 23b now covers the default auto + no-mux fallback alongside the existing forced-headless case (review-v9 finding 2, moderate).** v7's "headline" no-mux gate only tested `PI_SUBAGENT_MODE=headless` (forced). The plan's stated goal and architecture say backend selection falls back to headless in `PI_SUBAGENT_MODE=auto` (the default), and `selectBackend()` has real environment logic for that path (`pi-extension/subagents/cmux.ts:30-83`). Forced headless and auto/no-mux are adjacent but not the same path — a regression that only broke auto mode would still have passed the v7 gate. v8 keeps the forced-headless cases and adds two companion cases under a new sibling `describe` block that unsets `PI_SUBAGENT_MODE`, clears every mux env var (including `ZELLIJ_SESSION_NAME`), and invokes the same registered `subagent_serial` / `subagent_parallel` callbacks. A pass means `selectBackend()`'s auto fallback actually reaches headless when no mux is present — the behavior advertised in the goal.

**What was new in v7 (vs. v6):** the two findings from `.pi/plans/reviews/2026-04-20-mux-free-execution-design-review-v8.md` were remediated — see the "v7 changelog" section at the bottom of this document for the full list. In summary:

1. **Task 11's `runPiHeadless` no longer lies about pi → `TranscriptMessage` via a structural-subtyping cast (review-v8 finding 1, major).** v6 pushed every `message_end` payload (and every `tool_result_end` payload) into `transcript` via `msg as unknown as TranscriptMessage` and justified it as structural subtyping. That claim is false for pi `UserMessage` objects: pi-ai's `UserMessage.content` type is `string | (TextContent | ImageContent)[]` (`node_modules/@mariozechner/pi-ai/dist/types.d.ts:117-121`), while `TranscriptMessage.content` is declared `TranscriptContent[]`. A user message with `content: "hi"` therefore escaped as `content: string` through a field whose public type promised an array of typed blocks. v7 replaces the cast with a dedicated `projectPiMessageToTranscript(msg)` boundary projection that (a) normalizes `UserMessage` `content: string` into a single `[{type: "text", text}]` block, and (b) explicitly re-picks only the `TranscriptMessage` fields (`role`, `content`, plus `toolCallId` / `toolName` / `isError` for `toolResult`). A new unit test `test/orchestration/pi-transcript-projection.test.ts` exercises a real pi `message_end` user payload with `content: "hi"` and asserts the resulting `transcript[0].content` is a block array, not the original string. The architecture section's "structural subtyping" wording is also corrected — pi messages are *projected*, not structurally assigned.

2. **The pane-Claude E2E restriction test in Task 17 Step 5 now places `--print` before the `--` separator (review-v8 finding 2, moderate).** v6 built the command as `parts.join(" ") + " --print"`. Because `buildClaudeCmdParts` already terminates `parts` with `"--", shellEscape(task)` under v6's separator rule (`pi-extension/subagents/index.ts:687` after Task 17 Step 3), appending `--print` at the end places it *after* `--`, where it is parsed as another positional argument — not as the `--print` option. The reviewer verified locally: `claude --dangerously-skip-permissions -- 'Reply with exactly: OK' --print` hung, while `claude --dangerously-skip-permissions --print -- 'Reply with exactly: OK'` returned immediately. The test as written in v6 could therefore time out or measure the wrong mode. v7 changes the test to splice `--print` into `parts` immediately before the `--` separator before joining, so `--print` is parsed as an option. A defensive assertion in the test fails loud if the separator is not found, so a future refactor that drops the separator cannot silently regress the test into the broken v6 layout.

**What was new in v6 (vs. v5):** the three production-relevant findings and the two additional notes from `.pi/plans/reviews/2026-04-20-mux-free-execution-design-review-v7.md` were remediated — see the "v6 changelog" section at the bottom of this document for the full list. In summary:
...
**Claude + skills: explicitly out of scope for v1 (v6, review-v7 finding 3; v9, review-v10 finding 2).** `resolveLaunchSpec()` continues to compute `effectiveSkills` / `skillPrompts` for both CLIs because it is a pure normalization step. The pi backend consumes `skillPrompts` by passing them as positional `messages[1..]` to the pi CLI, where `/skill:foo` is a recognized prefix that expands skill content. The Claude backend (pane **and** headless) does **not** consume `skillPrompts` in v1: Claude's skills are plugin/slash-command based and are resolved by the Claude CLI when the model invokes them — pi's message-prefix convention does not port over. Prefixing Claude's `-p` prompt with literal `/skill:foo` strings would leak them as task content rather than expand them. When `cli: "claude"` is combined with non-empty `effectiveSkills`, **both** Claude backends emit a one-time `stderr` warning (`pi-interactive-subagent: ignoring skills=<list> on Claude path — not supported in v1`) and proceed without them. v9 (review-v10 finding 2) closes the pane-only silent-drop gap by threading the same `warnClaudeSkillsDropped(name, effectiveSkills)` helper into `launchSubagent()`'s Claude branch — the exact same function that `runClaudeHeadless` calls — so the pane and headless paths print identical wording and emit from a single source of truth. A follow-up spec can design Claude-specific skill delivery once Claude's slash-command behavior under `-p` mode is validated.

**Tech Stack:** TypeScript (Node's native `--test` runner, `node:assert/strict`), `@sinclair/typebox` for tool schemas, `@mariozechner/pi-coding-agent` for the extension API, `node:child_process` `spawn` for stdio-piped CLI launch. **v6:** `@mariozechner/pi-ai` is no longer added as a direct dependency — v5's `package.json` addition was only there to prop up the now-deleted `messages: Message[]` export (review-v7 finding 2). **v9 (review-v10 finding 1):** the pi headless backend also no longer imports pi-ai's `Message` type. v8 still had `import type { Message } from "@mariozechner/pi-ai"` inside `headless.ts` and typed `projectPiMessageToTranscript(msg: Message)` / `event.message as Message` against it, which only resolved under transitive hoisting through `@mariozechner/pi-coding-agent` — a package-resolution contract the fork does not own. v9 replaces that import with a local `PiStreamMessage` type defined next to the parser boundary, capturing exactly the fields the projection and usage accumulator read off the raw `message_end` / `tool_result_end` payloads. No pi-ai symbol crosses the backend boundary, so `package.json` does not need a direct or transitive dependency claim on `@mariozechner/pi-ai` to hold.
...
- `test/integration/orchestration-headless-no-mux.test.ts` — invokes the real `subagent_serial` / `subagent_parallel` registered tool callbacks in two environment setups: (a) forced `PI_SUBAGENT_MODE=headless` (original v3+ gate), and (b) **new in v8 (review-v9 finding 2)** `PI_SUBAGENT_MODE` unset + all mux env vars cleared, proving the orchestration entrypoint reaches headless both when forced *and* via `selectBackend()`'s default auto fallback.
 ...
 * a local `PiStreamMessage` struct, not an imported pi-ai `Message`). The Claude path emits a subset
...
/**
 * Second describe block (v9, review-v10 finding 2). The pane-Claude
 * skills-dropped warning must fire on the pane path, not just the headless
 * path. The warning is emitted synchronously from `launchSubagent()`'s
 * Claude branch before the child process is spawned, so we do NOT need to
 * invoke Claude or even check the filesystem here — we capture stderr
 * around a direct call to the shared helper. This intentionally does the
 * same capture in integration-test space (not just the unit test in
 * `claude-skills-warning.test.ts`) so a future refactor that silently
 * stops calling the helper from the pane path is caught by the integration
 * suite too. The unit test proves the helper works; this describe block
 * proves the pane-path call site still calls it.
 */
describe("pane-claude-skills-warning (review-v10 finding 2)", () => {
  // No skip: this is a pure stderr-capture test — it does not invoke Claude.
  it("warnClaudeSkillsDropped writes a single stderr line naming the subagent and skills", async () => {
    // Dynamic import lives inside the `it` body rather than at the top of
    // the file so the stderr shim is in place before the call. This avoids
    // any import-time warning (there is none today) contaminating the
    // capture. The package is ESM ("type": "module" in package.json), so
    // we use dynamic `import(...)` — not `require` — here.
    const { warnClaudeSkillsDropped } = await import(
    ...
    try {
      warnClaudeSkillsDropped("pane-subagent", "plan, code-review");
    } finally {
      (process.stderr as any).write = origWrite;
    }
    // Same wording contract as the unit test — this integration-level
    // check catches a regression where someone drops the import or the
    // call site inside launchSubagent() without noticing that only the
...
```bash
git add pi-extension/subagents/index.ts test/orchestration/thinking-effort.test.ts test/orchestration/claude-skills-warning.test.ts test/integration/pane-claude-tool-restriction.test.ts
git commit -m "feat(subagents): emit --tools + required -- separator on Claude path; share skills-dropped warning
...
exercises the full shell-escaped command path.
...
gap). Kept as a discrete named commit portable to an upstream
PR alongside the existing thinking patch."
```
...
### Task 23b: Integration test — `subagent_serial` / `subagent_parallel` reach headless in a no-mux environment (review finding 1; v8: auto-mode companion per review-v9 finding 2)

**Files:**
- Create: `test/integration/orchestration-headless-no-mux.test.ts`

This is the headline gate for review finding 1. The proposed unit tests instantiate `makeHeadlessBackend()` directly, but they do **not** prove that the real `subagent_serial` / `subagent_parallel` tool callbacks reach the headless backend after passing through `preflightOrchestration()`. This test exercises the full registered-tool path: it loads the extension via `subagentsExtension(pi)` against a captured-tools mock `pi`, then invokes the registered `subagent_serial` execute callback in two environments: (a) forced `PI_SUBAGENT_MODE=headless` with all mux env vars cleared, and (b) **new in v8 (review-v9 finding 2)** `PI_SUBAGENT_MODE` unset entirely (so the default `"auto"` path is exercised) with the same mux env vars cleared. The two configurations exercise adjacent-but-distinct paths through `selectBackend()` — forced mode returns `"headless"` directly, while auto mode runs the real mux-detection logic in `pi-extension/subagents/cmux.ts:30-83` before falling back to headless. A regression that only broke the auto path (e.g., a `selectBackend()` mux-detection bug, or a preflight that incorrectly required mux when mode was unset) would have passed the v7 gate and is now caught.
 ...
 * tool-execution result for the caller to assert on.
 */
async function runRegisteredTool(
  toolName: "subagent_serial" | "subagent_parallel",
  params: unknown,
  dir: string,
) {
...
describe("orchestration-headless-no-mux (auto + no mux env)", { skip: !PI_AVAILABLE, timeout: 180_000 }, () => {
  ...
  it("subagent_serial reaches headless via selectBackend's auto fallback when no mux is present", async () => {
    ...
    assert.notMatch(JSON.stringify(result), /mux not available/i,
      "review-v9 finding 2 regression: orchestration preflight blocked the auto + no-mux path");
    assert.equal(result.details.isError, false, `serial errored: ${JSON.stringify(result.details)}`);
    assert.equal(result.details.results.length, 1);
    assert.equal(result.details.results[0].exitCode, 0);
    assert.ok(result.details.results[0].finalMessage.trim().length > 0);
...
Run: `npm run test:integration -- --test-name-pattern='orchestration-headless-no-mux' 2>&1 | tail -60`
Expected: all four tests PASS (serial+parallel in forced-headless; serial+parallel in auto + no-mux), or all four skip when `pi` is missing. A regression of review finding 1 manifests as Case A failing with "review finding 1 regression: orchestration preflight blocked headless dispatch". A regression of review-v9 finding 2 manifests as Case B failing with "review-v9 finding 2 regression: orchestration preflight blocked the auto + no-mux path" — this is the path the feature is actually selling as the user default, so Case B must not be skipped or weakened when Case A passes.

- [ ] **Step 3: Commit**

```bash
git add test/integration/orchestration-headless-no-mux.test.ts
git commit -m "test(integration): exercise subagent_serial/parallel via real registered tool callbacks under both forced-headless and auto+no-mux"
```
...
The two remaining findings from `2026-04-20-mux-free-execution-design-review-v8.md` were addressed as follows:

**Finding 1 (Major) — pi headless `transcript` is no longer populated via an unsafe structural-subtyping cast**
- v6's `runPiHeadless` pushed every pi `message_end` payload (and every `tool_result_end` payload) into `transcript` via `msg as unknown as TranscriptMessage`, with a JSDoc block claiming structural subtyping made the assignment safe. That claim is false: pi-ai declares `UserMessage.content: string | (TextContent | ImageContent)[]` (`node_modules/@mariozechner/pi-ai/dist/types.d.ts:117-121`), while `TranscriptMessage.content: TranscriptContent[]`. A pi user message with `content: "hi"` therefore escaped through a public field that promised `content[0].type` would be defined — it wasn't.
- Task 11 Step 1 gains an exported `projectPiMessageToTranscript(msg: Message): TranscriptMessage` helper. The projection normalizes `UserMessage.content: string` into `[{ type: "text", text }]`, passes through assistant/toolResult block arrays (whose per-element shapes are a subset of `TranscriptContent`), and re-picks only the `TranscriptMessage` fields — so pi-ai's provider/usage/stopReason metadata cannot leak through the boundary.
- The two push sites inside `runPiHeadless` (`message_end` branch and `tool_result_end` branch) are rewritten to call `projectPiMessageToTranscript(...)` instead of casting. Usage / `stopReason` accumulation still reads off the original pi message because those fields are not part of the `TranscriptMessage` contract and feed the `usage` aggregate instead.
- Task 11 Step 1b is new: creates `test/orchestration/pi-transcript-projection.test.ts` with four cases — (1) user with `content: "hi"` projects to a block array; (2) assistant content blocks pass through unchanged and no pi-ai metadata leaks; (3) `toolResult` preserves `toolCallId` / `toolName` / `isError`; (4) defensive: a `toolResult` with `content: string` still lands as a block array. Regression of the v6 cast-based push fails this test immediately.
- Task 11 Step 3's commit list and commit message are updated to include the projection test and call out the finding in the body.
- The architecture section (top of file) changes its pi-side description from "assignable under structural subtyping" to "projected onto `TranscriptMessage` at the backend boundary", and File Structure adds the new projection test to the integration / orchestration test list.
- `headless.ts`'s type imports now include `TranscriptContent` from `./types.ts` so the projection can return a typed `TranscriptContent[]` without re-declaring the block union.

**Finding 2 (Moderate) — pane-Claude E2E restriction test now places `--print` before the `--` separator**
- v6's Task 17 Step 5 built the integration-test command as `parts.join(" ") + " --print"`. Because `buildClaudeCmdParts` ends `parts` with `"--", shellEscape(task)` under v5/v6's separator rule, `--print` appended to the end of the string was parsed as another positional argument, not the non-interactive-mode option. Locally reproduced by the reviewer: `claude --dangerously-skip-permissions -- 'Reply with exactly: OK' --print` hangs; `claude --dangerously-skip-permissions --print -- 'Reply with exactly: OK'` returns immediately. The v6 test could therefore time out or measure the wrong mode.
- v7 Task 17 Step 5 introduces a `withPrintBeforeSeparator(parts)` helper inside the test. The helper clones `parts`, finds the `--` separator (asserting `sepIdx > 0` so a future refactor that drops the separator fails loud instead of silently regressing), splices `--print` at `sepIdx`, and joins. Both test cases now build their command via this helper so `--print` is always parsed as an option.
- The Step 5 intro gains a "`--print` placement" paragraph that documents the bug, the on-host verification command pair, and the defensive assertion. The "Expected" block at the bottom of Step 5 is extended with a regression-of-finding-2 description (case 2 hangs up to the test timeout) so a future failure mode is legible without reopening this changelog.

---

## v8 changelog (vs. v7)

The two remaining moderate findings from `2026-04-20-mux-free-execution-design-review-v9.md` were addressed as follows:

**Finding 1 (Moderate) — pane-Claude E2E restriction test now asserts on a filesystem side effect, not assistant prose**
- v7's Task 17 Step 5 keyed both cases off `stdout.includes("HELLO_FROM_BASH_42")`: the restricted case asserted the marker was absent from the assistant's output, the baseline case asserted it was present. The reviewer flagged this as not a trustworthy oracle for a security-sensitive test: (a) a compliant refusal can legitimately quote the command it was asked to run — producing a false failure in the restricted case when tool restriction is actually working correctly; (b) the model can emit the marker text without actually invoking Bash (e.g., describing what the command would print) — producing a false pass in the baseline case when `--tools` actually broke.
- v8 Task 17 Step 5 rewrites both cases to drive off a filesystem side effect. Each case generates a per-case marker string (`Date.now()` + random suffix) and an absolute file path inside a per-test tempdir, then asks Claude to run `echo <marker> > <file>` via Bash. The restricted case asserts `!existsSync(file)` — the only way the file can exist is if Bash actually executed the redirection, and refusal prose (even if it quotes the command) cannot fake a file on disk. The baseline case asserts `existsSync(file)` **and** that `readFileSync(file, "utf8").trim()` equals the marker — so a baseline pass is only accepted if Bash not only ran, but ran with the exact side effect requested.
- The test file now uses a new `before` / `after` pair that `mkdtempSync(join(tmpdir(), "pi-pane-tool-restrict-"))` + `rmSync(..., { recursive: true, force: true })` so no test artifact is left on the host. A small `makeMarker(dir, label)` helper is added so the per-case marker / file pair is constructed in one line with uniqueness guaranteed even if parallel runs or repeated executions race each other inside the same tempdir.
- The restricted case wraps `execSync(cmd, ...)` in a `try { ... } catch { /* ok */ }` because a refusal-driven non-zero exit from Claude is acceptable under the new oracle — the assertion is about filesystem state, not subprocess exit code. The baseline case keeps the bare `execSync(...)` because a baseline failure to run at all is a distinct failure mode worth surfacing.
- The task text is rewritten to discourage emulation: "Run this exact bash command and nothing else: …" with a follow-up instruction in the baseline case ("Do not paraphrase the command; run it as written") and the restricted case ("If you cannot run it, say so briefly — do not describe or emulate the command"). These don't constrain the oracle's validity but do reduce test flakiness when the model prefers to narrate.
- The Step 5 intro gains a new "v8 (review-v9 finding 1) — side-effect oracle, not assistant prose" paragraph that documents the failure modes of the old oracle and the design of the new one. The "Expected" paragraph at the bottom of Step 5 is extended with a regression-of-v9-finding-1 sentence: "A regression of review-v9 finding 1 (reverting to a prose-based oracle) is caught at code review: this test must assert on `existsSync` / `readFileSync`, never on `stdout.includes(...)`."
- File Structure's `test/integration/pane-claude-tool-restriction.test.ts` entry is updated to mark the v8 switch in oracle style so a later reader of the top-of-file summary sees the change without having to diff Task 17 Step 5.
- The Task 17 commit list and commit message are otherwise unchanged — the test rewrite is a moderate tightening of an existing test that already landed in v6, not a new test file, so the upstream-portability story for the named commit (Task 17 ships `--tools` + `--` separator) is not affected.

**Finding 2 (Moderate) — Task 23b now covers the default auto + no-mux fallback alongside the existing forced-headless case**
- v7's `orchestration-headless-no-mux.test.ts` forced `process.env.PI_SUBAGENT_MODE = "headless"` in `before`, then ran the registered-tool assertions. That configuration makes `selectBackend()` return `"headless"` directly without consulting mux env vars at all — so the test passed whether or not the auto/no-mux code path in `selectBackend()` and `preflightOrchestration()` actually worked. The reviewer noted that the plan's goal and architecture both advertise headless as the user-default fallback in `PI_SUBAGENT_MODE=auto` (the default when the env var is unset), and that this is a distinct path through real environment logic (`pi-extension/subagents/cmux.ts:30-83`). A regression that only broke auto mode would still have passed the v7 gate.
- v8 Task 23b Step 1 keeps the forced-headless tests intact under a renamed describe block ("`orchestration-headless-no-mux (forced headless)`") and adds a sibling describe block ("`orchestration-headless-no-mux (auto + no mux env)`") with two new cases (`subagent_serial` and `subagent_parallel`). The new block's `before` hook save-and-clears the same full mux env-key list but does NOT set `PI_SUBAGENT_MODE`, so `selectBackend()` resolves to `"auto"` and runs the real mux-detection logic; with all mux env vars cleared, detection must report "no mux", and the auto fallback must land on headless. An explicit `assert.equal(process.env.PI_SUBAGENT_MODE, undefined, ...)` inside the `before` protects against a future edit silently turning this back into a forced-mode test.
- A new `MUX_ENV_KEYS` module-scope constant holds the list of env vars to save-and-clear so both describe blocks share exactly the same baseline, and a `runRegisteredTool(toolName, params, dir)` helper factors out the fake-pi construction and `process.chdir` dance so each `it(...)` body is a single expression plus its assertions. This keeps the new Case B tests lean (one screen each) and avoids a second copy of the 25-line fake-pi setup.
- The assertion-failure message in the new Case B tests references review-v9 finding 2 specifically ("`review-v9 finding 2 regression: orchestration preflight blocked the auto + no-mux path`") so a future failure can be traced back to this review without having to diff the describe block. The existing Case A message continues to reference the original review finding 1 (from review-v2).
- Task 23b's intro paragraph is rewritten to describe the two-configuration structure and to cite `cmux.ts:30-83` so the reader knows Case B exercises real environment logic, not a trivial constant. The "Expected" block in Step 2 is extended with regression descriptions for both review findings — including the note that Case B must not be skipped or weakened when Case A passes, because it is Case B that matches the user-default path.
- Task 23b's Step 3 commit message is updated to reflect both configurations: "exercise subagent_serial/parallel via real registered tool callbacks under both forced-headless and auto+no-mux".
- File Structure's `test/integration/orchestration-headless-no-mux.test.ts` entry is updated to mark the two-configuration coverage and to cite review-v9 finding 2 as the motivation for the auto-mode companion.

Neither finding required new production code changes — both are test-tightening. The Task 17 Step 5 oracle rewrite is a behavior-preserving change inside a test that was already landing in v6's named commit; the Task 23b auto-mode companion adds two new `it(...)` cases inside an already-existing test file. The rest of the plan — architecture, file structure (modulo the two line edits above), task sequencing, commit boundaries, phase gates, release-sequencing notes — is unchanged from v7.

---

## v9 changelog (vs. v8)

The two remaining moderate findings from `2026-04-20-mux-free-execution-design-review-v10.md` were addressed as follows:

**Finding 1 (Moderate) — pi-ai `Message` import removed from `headless.ts` in favor of a local `PiStreamMessage` type**
- v8 said "`@mariozechner/pi-ai` is no longer a direct dependency" (the v5 addition was removed in v6), but Task 11 Step 1 still contained `import type { Message } from "@mariozechner/pi-ai"` at the top of `pi-extension/subagents/backends/headless.ts`, typed `projectPiMessageToTranscript(msg: Message)` and `event.message as Message` (two sites: `message_end` and `tool_result_end`). The review pointed out that this only resolves under transitive hoisting through `@mariozechner/pi-coding-agent`, which is not a stable package-resolution contract — different installers, lockfile drift, or a pi-coding-agent peer-dep reshuffle could silently break the typecheck.
- Task 11 Step 1's code block now introduces a local `type PiStreamMessage = { role: "user" | "assistant" | "toolResult"; content: unknown; }` declaration immediately above the `projectPiMessageToTranscript` JSDoc. The type is a structural superset of pi-ai's `Message` union for the two fields the projection reads at the type level (`role`, `content`). Optional fields that exist only on specific pi-ai variants (`toolCallId` / `toolName` / `isError` on `toolResult`; `usage` / `stopReason` on `assistant`) stay off the type deliberately and are accessed via `(msg as any).…` — this narrows the contract to "exactly what the projection reads" and leaves a visible signal at every read site that this is a JSON-parsed payload, not a typechecked pi-ai object.
- `projectPiMessageToTranscript(msg: Message)` becomes `projectPiMessageToTranscript(msg: PiStreamMessage)`. The `msg.content` access is now unambiguous (no longer needs `(msg as any).content` for the initial read) because the field is declared `unknown` on the local type, matching the runtime shape.
- The two `event.message as Message` casts inside `processLine` are rewritten to `event.message as PiStreamMessage`. Inline comments cite review-v10 finding 1 so a later "clean-up" pass that tries to import the pi-ai type "for clarity" finds the rationale.
- The `import type { Message } from "@mariozechner/pi-ai"` line at the top of the file is removed and replaced with a block comment documenting why the import is absent.
- Task 11 Step 1b's unit test fixtures do not need shape changes — they were already typed `as any`. A new JSDoc block is added to the test describe block explaining that `as any` (not `as Message`) mirrors real usage where the caller feeds JSON-parsed payloads into the projection.
- Task 11 Step 3's commit message is extended with a paragraph attributing the local-type refactor to review-v10 finding 1.
- Architecture section "three layered changes" preamble now says v9 refines v6 finding (b) further so the pi headless backend does not import the pi-ai `Message` type at all. Tech Stack section is updated: v6's carefully-qualified "still imports pi-ai's `Message` internally, but this is a transitive resolution" sentence is replaced with a v9 note that the projection's parameter type is a local `PiStreamMessage` and no pi-ai symbol crosses the backend boundary. `package.json` stays as it is in v8 (no `@mariozechner/pi-ai` entry) — the review explicitly flagged transitive-hoist reliance as the problem, so the fix is to break the reliance, not to re-add the direct dep.
- File Structure's `test/orchestration/pi-transcript-projection.test.ts` entry notes the fixture-type change and cites review-v10 finding 1.

**Finding 2 (Moderate) — pane-Claude no longer silently drops `skills:`; shared `warnClaudeSkillsDropped` helper is called from both Claude paths**
- v8's architecture said "both Claude backends (pane and headless)" do not consume `skillPrompts` and emit a stderr warning, but only `runClaudeHeadless` (Task 19 Step 2) actually implemented the warning. `launchSubagent()`'s Claude branch (`pi-extension/subagents/index.ts:741-756`) never saw `effectiveSkills` after resolution — it was computed at `:708` and simply never read on the Claude path. A user launching `cli: "claude"` through the existing pane path still silently dropped `skills:`, which is exactly the confusing behavior the plan text said it wanted to avoid.
- Task 17 Step 3a is new: introduces `export function warnClaudeSkillsDropped(name, effectiveSkills): void` in `pi-extension/subagents/index.ts` and calls it from `launchSubagent()`'s Claude branch immediately before `buildClaudeCmdParts(...)`. The helper is a single-line no-op when `effectiveSkills` is undefined / null / empty / whitespace-only, so there is no user-visible noise when skills are not in play.
- Step 3a also adds a focused unit test `test/orchestration/claude-skills-warning.test.ts` that captures stderr around direct helper invocations and asserts: (a) a non-empty `effectiveSkills` produces a single stderr line that contains the subagent name, the skills list, and the phrase "Claude path" (the shared wording contract); (b) `undefined` produces no output; (c) empty / whitespace-only strings produce no output. This test is small (one `describe`, three `it`) but it catches wording drift between the pane and headless call sites on every test run.
- Task 17 Step 5's integration test file (`test/integration/pane-claude-tool-restriction.test.ts`) gets a second sibling `describe("pane-claude-skills-warning (review-v10 finding 2)", ...)` block with two cases: (1) stderr-capture around a direct `warnClaudeSkillsDropped` call — parallel to the unit test but lives in the integration file so a regression in the *call site inside `launchSubagent()`* (e.g. someone dropping the call without removing the unit test) surfaces here too; (2) a defensive `buildClaudeCmdParts` call with a normal task asserting that no `/skill:` token ever leaks into the pane argv — the builder has no skills field, but this locks the "skills never cross the builder boundary" contract. Both cases are skip-free because they do not invoke Claude — the warning fires synchronously at command-build time.
- Task 19 Step 2's `runClaudeHeadless` is rewritten to call `warnClaudeSkillsDropped(spec.name, spec.effectiveSkills)` instead of inlining the template string. A new `import { warnClaudeSkillsDropped } from "../index.ts"` is added to the top-of-file import list in Step 2. The "Claude + skills explicitly dropped with a stderr warning" paragraph that introduces Step 2 is updated to name the helper as the single source of truth and cite review-v10 finding 2.
- Task 17 Step 7's commit message is rewritten: the commit now ships both the `--tools` tool-restriction change (existing) and the shared skills-dropped warning helper + pane call site + unit test (new). The commit-message body adds a paragraph naming review-v10 finding 2 so the upstream-portability story is legible.
- File Structure's `pi-extension/subagents/index.ts` "carried changes" list gains a sixth entry for the `warnClaudeSkillsDropped` helper + pane call site. File Structure's `test/integration/pane-claude-tool-restriction.test.ts` entry is updated to mention the new sibling describe block. A new File Structure entry is added for `test/orchestration/claude-skills-warning.test.ts`.
- Architecture section "Claude + skills: explicitly out of scope for v1" subsection is updated: the sentence "the headless backend emits a one-time `stderr` warning" becomes "**both** Claude backends emit a one-time `stderr` warning". The shared `warnClaudeSkillsDropped` helper is named so a reader can navigate to the single source of truth in `index.ts`.
- README copy (in the final Task 27 README step) is updated so the "Skills" section says "both Claude backends emit the same warning" rather than "the headless backend emits a warning". Exact wording change is documented here so Task 27 does not have to re-derive it.

Finding 1 is a small code refactor (one type declaration added, one import removed, two parameter/cast sites updated) plus documentation updates. Finding 2 adds one new exported helper (~10 LOC in `index.ts`), one new call site in `launchSubagent()`'s Claude branch (~2 LOC), one new unit test file (~40 LOC), one new sibling describe block in an existing integration test (~50 LOC), and one-line call-site changes in `runClaudeHeadless`. Neither change is invasive, but each closes a behavior gap the plan text already promised to close.
