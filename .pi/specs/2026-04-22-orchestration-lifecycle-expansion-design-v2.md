...
type OrchestratedTaskResult = {
   ...
   * the Claude session id. Named "sessionKey" (not "sessionId") because the
   * pi-backed form is path-shaped, not id-shaped.
   */
  sessionKey?: string;
...
The `sessionKey` is the same value the parent will pass back through `subagent_resume`. For pi-backed children that's the subagent session file path (so it matches `subagent_resume({ sessionPath })` directly); for Claude-backed children it's the Claude session id. Named `sessionKey` rather than `sessionId` because the pi-backed form is path-shaped, not id-shaped.
...
The parent calls the standalone `subagent_resume` tool directly with the blocked session's `sessionKey` (as `sessionPath` for pi-backed children — they are the same string) and the answer as the follow-up prompt. This is the only unblock path; no orchestration-level unblock tool is introduced.
